...
impl AudioEncoderLayer {
    ...
    pub fn forward(&self, x: &Array) -> Result<Array, String> {
        ...
        // QKV projections.
        let q = self.q_proj.forward(&normed).map_err(|e| format!("q_proj: {:?}", e))?;
        let k = self.k_proj.forward(&normed).map_err(|e| format!("k_proj: {:?}", e))?;
        ...
        let scores = q.matmul(&ops::transpose_axes(&k, &[0, 2, 1])?)?
            .divide(&Array::from_f32(scale))?;
        let attn_weights = ops::softmax_axes(&scores, &[-1], None)
        ...
        let x = ops::add(residual, &attn_out)
        ...
        let gated = ops::multiply(&nn::silu(&gate)?, &up)
            .map_err(|e| format!("silu*gate: {:?}", e))?;
        let ffn_out = self.down_proj.forward(&gated)
            .map_err(|e| format!("down_proj: {:?}", e))?;

        let result = ops::add(&x, &ffn_out)
            .map_err(|e| format!("ffn residual: {:?}", e))?;
        result.eval()?;
        Ok(result)
    }
    ...
    fn rms_norm(&self, x: &Array, weight: &Array) -> Result<Array, String> {
        ...
        primitives::rms_norm(x, weight, eps)
...
impl AudioEncoder {
    ...
    pub fn load(model: &LoadedProfiledModel) -> Result<Self, String> {
            ...
            match entry {
                ...
                None => {
                    ...
                    let entry = model
                        ...
                        .tensor_table
                        .iter()
                        .find(|e| e.name == name.replace(".scales", ".weight").replace(".biases", ".weight"))
                        .ok_or_else(|| format!("weight tensor not found for side tensor: {}", name))?;
                    let shape: Vec<i32> = entry.logical_shape.iter().map(|&d| d as i32).collect();
                    let out_dim = shape[0];
                    let n_groups = 1;
                    let dummy = Array::from_slice(&vec![0.0f32; (out_dim * n_groups) as usize], &[out_dim, n_groups]);
                    let dummy_biases = Array::from_slice(&vec![0.0f32; (out_dim * n_groups) as usize], &[out_dim, n_groups]);
                    if name.ends_with(".scales") {
                        Ok(crate::bridge::ARRAY_REGISTRY.write().insert(dummy, None))
                    } else {
                        Ok(crate::bridge::ARRAY_REGISTRY.write().insert(dummy_biases, None))
                    }
                }
            }
        };
        ...
        let find_tensor = |name: &str| -> Result<(u64, u64, u64, u32, u32), String> {
            ...
            Ok((handle, scales_handle, biases_handle, out_features, in_features))
        };
        ...
        let input_proj = QuantizedLinearBinding::new(
            w_h, s_h, b_h, out_feat, in_feat, 64, 8, true,
        );
        ...
        for layer_idx in 0..num_layers {
            ...
            let load_qkv = |proj: &str| -> Result<QuantizedLinearBinding, String> {
                ...
                Ok(QuantizedLinearBinding::new(w, s, b, out_f, in_f, 64, 8, true))
            };
        ...
        let (w_h, s_h, b_h, out_f, in_f) = find_tensor(&output_proj_name)
            .unwrap_or_else(|_| (0, 0, 0, hidden_size, hsize));

        let output_proj = if w_h != 0 {
            Some(QuantizedLinearBinding::new(w_h, s_h, b_h, out_f, in_f, 64, 8, true))
        } else {
