...
fn load_audio_from_file(path: &str, target_sample_rate: u32) -> Result<Vec<f32>, String> {
    ...
    match ext.as_str() {
        ...
        other => Err(format!("Unsupported audio format: {} (only WAV supported)", other)),
    }
}
        ...
        match &chunk_id {
            b"fmt " => {
                ...
                bits_per_sample = u16::from_le_bytes(
                    buf[offset + 14..offset + 16].try_into().unwrap(),
                );
            ...
            let sample_f32 = match bits_per_sample {
                ...
                16 => {
                    let val = i16::from_le_bytes(
                        data[ch_offset..ch_offset + 2].try_into().unwrap(),
                    );
                ...
                32 => {
                    if byte_depth == 4 {
                        ...
                        let val = i32::from_le_bytes(
                            data[ch_offset..ch_offset + 4].try_into().unwrap(),
                        );
