...
impl Arena {
    ...
    /// Allocate a 1D IOSurface-backed buffer of `byte_count` bytes.
    /// The memory is zero-initialized and accessible via [`base_ptr`].
    /// This is the general-purpose allocation for weight tensors,
    /// accepting any dtype and shape.
    pub fn new_bytes(byte_count: u32) -> Result<Self, String> {
        if byte_count == 0 {
            return Err("Arena::new_bytes: byte_count must be > 0".into());
        }
        let mut info: ArenaInfo = unsafe { std::mem::zeroed() };
        let rc = unsafe { tribunus_arena_alloc_bytes(&mut info, byte_count as i32) };
        if rc != 0 {
            return Err(format!("tribunus_arena_alloc_bytes failed: {}", rc));
        }
        Ok(Arena {
            info,
            dtype: mlx_rs::Dtype::Float32, // logical dtype irrelevant for byte storage
            externally_owned: true,
        })
    }
    ...
    /// Evaluate an MLX array so its result materializes directly into
    /// this arena's IOSurface backing store.
    ///
    /// Sets the MLX Metal allocator's output buffer hint to this arena's
    /// memory pointer and size, then evaluates `array`.  The first
    /// allocation whose size exactly matches `self.byte_len()` will be
    /// redirected to this arena instead of allocating fresh Metal heap
    /// memory, achieving zero-copy transport from MLX into the IOSurface.
    ///
    /// Intermediate tensors in the compute graph still use the normal
    /// Metal allocator; only the final output lands in the arena.
    ///
    /// # Safety
    ///
...
impl OutputBufferHint for Arena {
    fn buffer_ptr(&self) -> *const std::ffi::c_void {
        self.info.base_address as *const std::ffi::c_void
    }
...
mod tests {
    ...
    fn test_arena_ping_pong() {
        ...
        eprintln!(
            "[arena] ping-pong alloc: PASS (ids {} / {})",
            a.io_surface_id(),
            b.io_surface_id()
        );
    }
    ...
    fn test_iosurface_phase1_mlx_external() {
        ...
        let arr = unsafe {
            let storage = Arc::new(external_array::StaticStorage::new(
    ...
    fn test_iosurface_phase3_output_backings() {
        ...
        assert_eq!(
            arena_b.info.base_address, b_addr_before,
    ...
    fn test_gemma_mlp_coreml_prediction() {
        ...
        assert!(
            found_nonzero,
            "all Gemma MLP output elements are zero (dead model?)"
        );
