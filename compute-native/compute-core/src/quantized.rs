...
mod tests {
    ...
    fn test_concurrent_independent_mlx_execution() {
        ...
        eprintln!("[concurrent-qual] 2 threads x {n} quantized_matmul iterations: PASS");
    }
    ...
    fn test_concurrent_heavy_mlx_stress() {
        ...
        for _t in 0..4 {
            handles.push(std::thread::spawn(move || {
                for _i in 0..iterations {
                    ...
                    let q = mlx_rs::ops::quantized_matmul(&x, &w, &s, &b, true, group_size, bits)
        ...
        eprintln!("[concurrent-stress] 4 threads x {iterations} heavy quantized_matmul: PASS");
    }
}
