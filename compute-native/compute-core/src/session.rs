...
mod tests {
    ...
    fn test_control_state_identity_transitions() {
        ...
        for s in states {
            assert!(
                s.transition(s).is_ok(),
                "Identity transition should be valid for {:?}",
                s
            );
        }
    }
    ...
    fn test_control_session_fail_from_non_terminal() {
        for (label, mut session) in [
            (
                "created",
                GenerationControlSession::new("f1".into(), 2, 100),
            ),
            ("admitted", {
    ...
    fn test_inference_session_state_valid_transitions() {
        assert!(InferenceSessionState::Created
            .transition(InferenceSessionState::PrefillRunning)
            .is_ok());
        assert!(InferenceSessionState::PrefillRunning
            .transition(InferenceSessionState::Decoding)
            .is_ok());
        assert!(InferenceSessionState::Decoding
            .transition(InferenceSessionState::Completed)
            .is_ok());
        // Cancellation
        assert!(InferenceSessionState::Decoding
            .transition(InferenceSessionState::Cancelled)
            .is_ok());
        assert!(InferenceSessionState::PrefillRunning
            .transition(InferenceSessionState::Cancelled)
            .is_ok());
    ...
    fn test_inference_session_state_terminal_rejects_failed() {
        ...
        assert!(
            InferenceSessionState::Completed
                .transition(InferenceSessionState::Failed)
                .is_err(),
            "Failed from Completed should be rejected",
        );
        assert!(
    ...
    fn test_inference_session_state_invalid_transitions() {
        ...
        assert!(InferenceSessionState::Completed
            .transition(InferenceSessionState::PrefillRunning)
            .is_err());
        assert!(InferenceSessionState::Cancelled
            .transition(InferenceSessionState::Created)
            .is_err());
        assert!(InferenceSessionState::Failed
            .transition(InferenceSessionState::Created)
            .is_err());
    }
