...
impl fmt::Debug for GenerationStream {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("GenerationStream")
            ...
            .field(
                "disconnect_tx",
                &self.disconnect_tx.as_ref().map(|_| "<Sender>"),
            )
            .field(
                "disconnect_rx",
                &self.disconnect_rx.as_ref().map(|_| "<Receiver>"),
            )
...
impl GenerationStream {
    ...
    pub fn take_disconnect_notifier(&mut self) -> Option<tokio::sync::oneshot::Receiver<()>> {
...
impl GenerationSender {
    ...
    pub fn try_send(&self, event: GenerationEvent) -> NapiResult<()> {
        let mut guard = self.shared.queue.lock();
        if self.shared.closed.load(Ordering::Acquire) {
            return Err(NapiError::from_reason("channel closed"));
        }
        if guard.len() >= self.shared.capacity {
            return Err(NapiError::from_reason("channel full"));
        }
    ...
    pub fn try_send_or_cancel(&self, event: GenerationEvent) -> NapiResult<()> {
        let mut guard = self.shared.queue.lock();
        if self.shared.closed.load(Ordering::Acquire) {
            return Err(NapiError::from_reason("Cancelled"));
        }
        if guard.len() >= self.shared.capacity {
            return Err(NapiError::from_reason("channel full"));
        }
...
/// ```
#[cfg_attr(feature = "napi", napi)]
pub fn generation_channel(capacity: Option<u32>) -> (GenerationSender, GenerationStream) {
...
mod tests {
    ...
    fn test_sequence_no_started_rejects_tokens() {
        ...
        let events = vec![GenerationEvent::Token(42), GenerationEvent::Done];
    ...
    fn test_sequence_missing_terminal() {
        let events = vec![GenerationEvent::Started, GenerationEvent::Token(42)];
        assert!(!validate_event_sequence_impl(&events));
    }
    ...
    fn test_event_sequence_validation() {
        ...
        // Valid: only terminal
        assert!(validate_event_sequence_impl(&[GenerationEvent::Cancelled,]));
