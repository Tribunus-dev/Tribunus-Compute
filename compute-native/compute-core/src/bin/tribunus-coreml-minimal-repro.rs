...
fn build_meta(contract: &DiagnosticGraphContract) -> ModelMeta {
    let inputs: Vec<(String, Vec<i64>)> = vec![("x".to_string(), vec![1, contract.shape_k as i64])];
    let outputs: Vec<(String, Vec<i64>)> = contract
        .output_shapes
        .iter()
        .enumerate()
        .map(|(i, shape)| {
...
fn run_graph(config: &RunConfig, contract: &DiagnosticGraphContract) -> GraphOutcome {
    ...
    match compile_mlpackage(&pkg_path, &graph_dir, &island_id) {
        Ok((exit_code, stdout, stderr, duration_ns)) => {
            ...
            GraphOutcome {
                ...
                load_status: if load_ok {
                    "available".to_string()
                } else {
                    "not_found".to_string()
                },
                failure_diagnostics: if exit_code != 0 {
                    Some(stderr.clone())
                } else {
                    None
                },
...
fn main() {
    ...
    let pass = outcomes
        .iter()
        .filter(|o| o.compile_status == "pass" || !o.compile_invoked)
        .count();
    let fail = outcomes
        .iter()
