...
fn print_usage() {
    eprintln!("Usage:");
    eprintln!("  tribunus-bench prefill  --model <path> [--prompt-tokens 64] [--warmup 3] [--trials 5]");
...
fn do_prefill_mlx(
    ...
    let model = LoadedProfiledModel::new(model_path)
        .expect("failed to load model for prefill bench");
    ...
    if n > 0.0 {
        ...
        metrics.insert("prefill_time_median_ms".into(), serde_json::json!(median_ms));
        ...
        metrics.insert("prefill_tokens_per_sec".into(), serde_json::json!(prefill_tok_s));
        metrics.insert("prefill_tokens_per_sec_min".into(), serde_json::json!(prefill_tok_s_min));
        metrics.insert("prefill_tokens_per_sec_max".into(), serde_json::json!(prefill_tok_s_max));
    }
...
fn do_decode_mlx(
    ...
    let load_start = Instant::now();
    let model = LoadedProfiledModel::new(model_path)
        .expect("failed to load model for decode bench");
    let load_ms = load_start.elapsed().as_secs_f64() * 1000.0;
    metrics.insert("model_load_ms".into(), serde_json::json!(load_ms));
    ...
    // Benchmark: single prefill + N decode steps
    {
        ...
        metrics.insert("decode_tokens_per_sec".into(), serde_json::json!(decode_tok_s));
        metrics.insert("kv_cache_estimate_bytes".into(), serde_json::json!(total_kv_bytes));
        metrics.insert("kv_cache_estimate_mib".into(), serde_json::json!(total_kv_bytes as f64 / (1024.0 * 1024.0)));
    }
...
fn do_serve_mlx(
    ...
    let prompt: Arc<Vec<u32>> = Arc::new(
        (0..prompt_tokens).map(|i| (i % 1024) as u32 + 1).collect(),
    );
        ...
        handles.push(std::thread::spawn(move || {
            loop {
                ...
                let kv_caches: Vec<KvCache> = (0..n_layers)
                    ...
                    .collect();
                let mut session =
                    ProfiledInferenceSession::new("serve-request".into(), kv_caches);
                session.setup_from_model(&model);
    ...
    metrics.insert("total_decode_tokens".into(), serde_json::json!(total_decoded));
    metrics.insert("throughput_requests_per_sec".into(), serde_json::json!(throughput_rps));
    metrics.insert("throughput_tokens_per_sec".into(), serde_json::json!(throughput_tok_s));
...
fn cmd_compare(args: &[String]) {
    ...
    for bf in &baseline_files {
        ...
        if let (Some(bm), Some(cm)) = (b_metrics.as_object(), c_metrics.as_object()) {
            ...
            let all_keys: std::collections::BTreeSet<&str> = bm
                .keys()
                .chain(cm.keys())
                .map(String::as_str)
                .collect();
