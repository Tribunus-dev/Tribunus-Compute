...
fn print_usage() {
    eprintln!("Usage: tribunus-bench-smoke --model-path <path> [--prompt <text>] [--max-tokens <N>]");
    eprintln!();
    eprintln!("Options:");
...
fn main() {
    ...
    let prompt_tokens: Vec<u32> = if let Some(ref tok) = &tokenizer {
        match tok.encode(&prompt) {
            ...
            Err(e) => {
                log_error!("Tokenizer encode failed: {}, falling back to byte tokenizer", e);
