...
fn main() {
    ...
    if single_row {
        ...
        let family = MEASUREMENT_FAMILIES
            .iter()
        ...
        let last_crumb = tribunus_compute_core::decode_attribution::breadcrumb::last_breadcrumb(
            crumb_path_for_read,
        )
        .unwrap_or_default();
    ...
    let env: HostEnvironment =
        environment::capture_host_environment().unwrap_or_else(|_| HostEnvironment {
    ...
    tribunus_compute_core::log_info!(
        ...
        parent_cal.timer_overhead_method
    );
...
fn run_coreml_row_in_child(
    ...
    });
    {
        ...
        f.write_all(row_desc.to_string().as_bytes())
            .expect("write crash repro");
    }
    ...
    let child_output = Command::new(self_path)
        .args(["--single-row", backend, family.name, shape.name, policy])
    ...
    match child_output {
            ...
            if status.success() {
                ...
                if let Some(receipt_json) = extract_receipt(&stdout) {
                    ...
                    synthetic_child_failure(
                        row_id,
                        "measurement_failed",
                        "no receipt marker in child stdout",
                        status,
                    )
                }
            } else {
        ...
        Err(e) => synthetic_child_failure(
            row_id,
            "measurement_failed",
            &format!("child spawn: {e}"),
...
fn classify_child_exit(
    ...
    let last_crumb =
        tribunus_compute_core::decode_attribution::breadcrumb::last_breadcrumb(&crumb_path)
            .unwrap_or_default();
    ...
    r.commit_sha = option_env!("VERGEN_GIT_SHA")
        .unwrap_or("unknown")
        .to_string();
    r.branch = option_env!("VERGEN_GIT_BRANCH")
        .unwrap_or("unknown")
        .to_string();
...
fn synthetic_child_failure(
    ...
    let mut r = DecodeAttributionReceipt::default();
    r.run_id = row_id.to_string();
    r.mark_predict_blocked("unknown", diag.into());
    r.status = status.into(); // Override status if needed
    r.commit_sha = option_env!("VERGEN_GIT_SHA")
        .unwrap_or("unknown")
        .to_string();
    r.branch = option_env!("VERGEN_GIT_BRANCH")
...
fn synthetic_env_unavailable_receipt(
    ...
    DecodeAttributionReceipt {
        ...
        failure_diagnostics: Some(format!(
            "coremlcompiler unavailable: {}",
            env.coremlcompiler_version
        )),
