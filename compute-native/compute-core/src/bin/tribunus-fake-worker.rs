...
use tribunus_compute_core::worker_protocol::{
    Frame, GenerationCompletedPayload, HostCommand, MessageKind, TokenPayload, WorkerEvent,
...
fn do_handshake<R: Read, W: Write>(reader: &mut R, writer: &mut W, worker_id: &str, seq: &mut u64) {
    let frame = read_frame(reader).expect("read Hello");
    assert!(
        is_host_command(&frame, HostCommand::Hello),
        "expected Hello"
    );
    write_frame(
        writer,
...
fn do_load_model<R: Read, W: Write>(
    ...
    assert!(
        is_host_command(&frame, HostCommand::LoadModel),
        "expected LoadModel"
    );
    write_frame(
        writer,
