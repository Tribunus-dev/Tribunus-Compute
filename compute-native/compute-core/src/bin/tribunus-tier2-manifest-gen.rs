...
fn main() {
    ...
    serde_json::to_writer_pretty(
        std::fs::File::create(&claimed_matrix_path).unwrap(),
    ...
    for row in &manifest.rows {
        ...
        let blocked = if row.family.contains("residual")
    ...
    serde_json::to_writer_pretty(
        std::fs::File::create(&matrix_path).unwrap(),
        &matrix_entries,
    )
    .unwrap_or_else(|e| {
    ...
    let native_count = matrix_entries
        ...
        .count();
    let composed_count = matrix_entries
        .iter()
        .filter(|e| e.support_status == "supported_domain_adapter")
        .count();
    let pending_count = matrix_entries
        .iter()
        .filter(|e| e.support_status.starts_with("pending"))
        .count();
    let unsupported_count = matrix_entries
        .iter()
        .filter(|e| e.support_status.starts_with("unsupported"))
        .count();
    let blocked_count = matrix_entries
        .iter()
