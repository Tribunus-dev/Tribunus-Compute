...
mod tests {
    ...
    fn test_f16_from_bits() {
        ...
        assert!((one_pt_125 - 1.125).abs() < 1e-3, "expected 1.125, got {one_pt_125}");

        // Denormalized: smallest positive subnormal
        let tiny = f16_from_bits(0x0001);
        assert!(tiny > 0.0 && tiny < 1e-6, "expected tiny positive, got {tiny}");
    }
    ...
    fn test_dequantize_ggml_q4_0_roundtrip() {
        ...
        for row in 0..n_rows {
            for b in 0..block_count {
                let offset = row * block_count * BlockQ4_0::BYTES_PER_BLOCK
                    + b * BlockQ4_0::BYTES_PER_BLOCK;
                // Scale = row+1 (in fp16)
