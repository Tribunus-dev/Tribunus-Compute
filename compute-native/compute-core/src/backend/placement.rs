...
impl HazardBarrier {
    pub fn new(view: ArenaView) -> Self {
        HazardBarrier { view, last_writer: None, pending_readers: Vec::new() }
    }
...
impl HazardTracker {
    pub fn new() -> Self {
        HazardTracker { barriers: std::collections::HashMap::new() }
    }

    pub fn register_view(&mut self, view: ArenaView) {
        self.barriers.entry(view.0).or_insert_with(|| HazardBarrier::new(view));
    }
    ...
    pub fn needs_sync(&mut self, view: ArenaView, lane: ExecutionLane) -> bool {
        self.barriers.get_mut(&view.0).map(|b| b.read(lane)).unwrap_or(false)
    }
