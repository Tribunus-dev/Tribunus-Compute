...
fn get_available_l2_size() -> usize {
    #[cfg(target_os = "macos")]
    {
        ...
        let rc = unsafe { libc::sysctl(mib.as_mut_ptr(), 2, &mut size as *mut _ as *mut libc::c_void, &mut len, std::ptr::null_mut(), 0) };
        if rc == 0 && size > 0 {
...
impl CpuAttentionScheduler {
    ...
    pub fn new(
        ...
        let max_q_heads_per_iter =
            max_q_heads_per_iter.unwrap_or(q_per_kv.clamp(1, 8));
    ...
    fn calc_default_tile_tokens(&self) -> usize {
        ...
        let denom = hd * (q_buf_elem + 2 * elem + out_elem)
            + self.max_q_heads_per_iter * logits_elem;
    ...
    fn align_kv_up(pos: usize) -> usize {
        ((pos + BLOCK_SIZE_ALIGNMENT - 1) / BLOCK_SIZE_ALIGNMENT)
    ...
    pub fn schedule(
        ...
        let min_split_kv_len = ((self.max_q_heads_per_iter * 4
            + BLOCK_SIZE_ALIGNMENT
            - 1)
            / BLOCK_SIZE_ALIGNMENT)
            * BLOCK_SIZE_ALIGNMENT;
        ...
        for req_idx in 0..self.batch_size {
            ...
            while q_tile_offset < q_tokens {
                let q_tile_tokens =
                    (q_tokens - q_tile_offset).min(max_q_tokens_per_iter);
                ...
                let kv_aligned = Self::align_kv_up(kv_tile_right)
                    - Self::align_kv_down(kv_tile_left);
                ...
                while kv_remaining > 0 {
                    ...
                    if remaining_kv < min_split_kv_len as i64
                        && counts_per_thread[thread_idx] > 0
                    {
        ...
        AttentionScheduleResult {
            work_items,
            work_item_ranges: (0..effective)
                .map(|i| (ranges[i], ranges[i + 1]))
                .collect(),
    ...
    pub fn execute_attention<F>(
        schedule: &AttentionScheduleResult,
        work_fn: F,
    ) where
...
mod tests {
    ...
    fn test_schedule_decode_batch() {
        ...
        assert!(!schedule.work_items.is_empty());
        assert_eq!(
            schedule.effective_threads,
            schedule.work_item_ranges.len()
        );
    }
    ...
    fn test_execute_attention_work_stealing() {
        ...
        assert_eq!(visited.load(std::sync::atomic::Ordering::Relaxed), schedule.work_items.len());
    }

    #[test]
    fn test_default_tile_tokens_nonzero() {
        let scheduler = CpuAttentionScheduler::new(
            1, 32, 8, 128, None, None, false,
        );
