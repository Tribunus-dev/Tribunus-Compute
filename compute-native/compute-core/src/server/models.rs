...
impl ModelRegistry {
    ...
    pub async fn download_hf(repo: &str, filename: &str, cache_dir: &PathBuf) -> Result<PathBuf, String> {
    ...
    pub fn compile(&mut self, id: &str, source_dir: &PathBuf) -> Result<(), String> {
        ...
        let _ = id;
        let _ = source_dir;
        Err("ComputeImage compilation: use the existing compute_image::compile route in server".into())
    }
