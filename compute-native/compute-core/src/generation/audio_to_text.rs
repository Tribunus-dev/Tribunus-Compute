...
impl AudioToTextGenerator {
    ...
    pub fn load(image_path: &str) -> Result<Self, String> {
        ...
        let mut paraformer =
            funasr_mlx::paraformer::load_model(&weights_path)
        ...
        let vocab =
            Vocabulary::load(&vocab_path).map_err(|e| format!("load vocabulary: {e}"))?;
    ...
    pub fn transcribe(
        ...
        let (samples, _sample_rate): (Vec<f32>, u32) =
            load_audio_for_paraformer(audio_path)
                .map_err(|e| format!("load audio: {e}"))?;
