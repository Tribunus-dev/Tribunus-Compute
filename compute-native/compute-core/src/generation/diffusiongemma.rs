...
impl DiffusionSampler {
    ...
    pub fn generate(
        ...
        self.generate_with_params(model, prompt_tokens, max_new_tokens, steps, DEFAULT_MASK_THRESHOLD)
    }
    ...
    pub fn generate_with_thinking(
        ...
        self.generate_with_params(model, prompt_tokens, max_new_tokens, steps, THINKING_MASK_THRESHOLD)
    }
    ...
    pub fn generate_draft(
        ...
        self.generate_with_params(model, prompt_tokens, draft_length, steps, DEFAULT_MASK_THRESHOLD)
    }
    ...
    fn generate_with_params(
        ...
        while (all_tokens.len() as u32) < prompt_tokens.len() as u32 + max_new_tokens {
            ...
            let remaining = (prompt_tokens.len() as u32 + max_new_tokens).saturating_sub(all_tokens.len() as u32);
            ...
            let (new_tokens, _is_confident) = self.decode_tokens(&final_latents, &confidence, mask_threshold)?;
    ...
    fn sample_noise(&self, n: u32, step: u32, total_steps: u32) -> Result<Array, String> {
        ...
        Array::from_slice(&data, &[n as i32, hidden_size])
            .map_err(|e| format!("failed to create noise array: {:?}", e))
    }

    /// Add noise scaled by the schedule to the latent representation.
    fn add_noise(&self, latents: &[f32], step: u32, total_steps: u32, seed: u64) -> Result<Vec<f32>, String> {
        let sigma = schedule_sigma(step, total_steps, self.noise_schedule);
    ...
    fn denoise_step(
        ...
        let confidence: Vec<f32> = (0..n_positions)
            .map(|_| alpha.min(0.95) + 0.05)
            .collect();
...
impl DiffusionModel {
    ...
    pub fn load(image_dir: &str) -> Result<Self, String> {
        ...
        let diffusion_config = if config_path.exists() {
            ...
            DiffusionConfig {
                max_diffusion_tokens: raw.get("max_diffusion_tokens").and_then(|v| v.as_u64()).unwrap_or(256) as u32,
                default_denoising_steps: raw.get("default_denoising_steps").and_then(|v| v.as_u64()).unwrap_or(6) as u32,
                ...
                parallel_token_generation: raw.get("parallel_token_generation").and_then(|v| v.as_u64()).unwrap_or(18) as u32,
                supports_images: raw.get("supports_images").and_then(|v| v.as_bool()).unwrap_or(true),
                supports_video: raw.get("supports_video").and_then(|v| v.as_bool()).unwrap_or(true),
                image_size: raw.get("image_size").and_then(|v| v.as_u64()).unwrap_or(896) as u32,
                patch_size: raw.get("patch_size").and_then(|v| v.as_u64()).unwrap_or(16) as u32,
                max_context_length: raw.get("max_context_length").and_then(|v| v.as_u64()).unwrap_or(262_144) as u32,
            }
        } else {
    ...
    pub fn chat(
        ...
        let generated_tokens = if self.is_thinking_request(messages) {
            self.sampler.generate_with_thinking(self, &prompt_tokens, max_tokens)?
        } else {
            self.sampler.generate(self, &prompt_tokens, max_tokens, None)?
        };
    ...
    fn format_chat_prompt(
        ...
        for msg in messages {
            ...
            match role.as_str() {
                ...
                _ => {
                    prompt.push_str(&format!("<|{}|>\n{}\n", role, self.format_content_parts(&msg.content)));
                }
            }
        }
    ...
    pub fn call_function(
        ...
        let mut full_prompt = String::from(
            "You are a function-calling assistant. Given the available tools, "
        );
        ...
        let generated = self.sampler.generate_with_thinking(self, &prompt_tokens, 512)?;
            ...
            (Some(start), Some(end)) if end > start => {
                ...
                match serde_json::from_str::<serde_json::Value>(json_str) {
                    ...
                    Err(e) => {
                        ...
                        Err(format!("failed to parse function call JSON: {e}. Raw output: {text}"))
                    }
                }
            }
    ...
    pub fn generate_code(&self, prompt: &str) -> Result<String, String> {
        ...
        let generated = self.sampler.generate_with_thinking(self, &prompt_tokens, 1024)?;
    ...
    pub fn generate_with_params(
        ...
        self.sampler.generate_with_params(self, prompt_tokens, max_new_tokens, steps, mask_threshold)
    }
    ...
    pub fn generate(
        ...
        if width < MIN_IMAGE_SIZE || height < MIN_IMAGE_SIZE || width > MAX_IMAGE_SIZE || height > MAX_IMAGE_SIZE {
            return Err(format!(
...
mod tests {
    ...
    fn test_noise_schedule_cosine() {
        ...
        assert!((alpha_start - 1.0).abs() < 0.01, "alpha_start={} should be ~1.0", alpha_start);
        assert!(alpha_end < 0.1, "alpha_end={} should be near 0.0", alpha_end);
    }
    ...
    fn test_noise_schedule_linear() {
        ...
        assert!((alpha_start - 1.0).abs() < 0.01, "alpha_start={} should be ~1.0", alpha_start);
        assert!((alpha_end - 0.0).abs() < 0.01, "alpha_end={} should be ~0.0", alpha_end);
    }
    ...
    fn test_schedule_sigma() {
        ...
        assert!(sigma_start < 0.01, "sigma_start={} should be near 0.0", sigma_start);
        assert!((sigma_end - 1.0).abs() < 0.01, "sigma_end={} should be ~1.0", sigma_end);
    }
