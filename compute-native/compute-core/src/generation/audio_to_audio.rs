...
impl AudioToAudioGenerator {
    ...
    pub fn load(image_path: &str) -> Result<Self, String> {
        ...
        if !path.is_dir() {
            return Err(format!(
                "audio model image not found at {}",
                path.display()
            ));
        }
    ...
    pub fn voice_clone(
        ...
        let mut prompt_tokens: Vec<u32> = Vec::with_capacity(
            4 + text.len() + reference_audio.len() / 64,
        );
    ...
    pub fn style_transfer(
        ...
        let mut prompt_tokens: Vec<u32> = Vec::with_capacity(
            4 + style_prompt.len() + input_audio.len() / 64,
        );
    ...
    fn tokens_to_audio(&self, tokens: &[u32]) -> Result<Vec<f32>, String> {
        ...
        let token_mean = tokens.iter().copied().map(|t| t as f32).sum::<f32>()
            / tokens.len() as f32;
