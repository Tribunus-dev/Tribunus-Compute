...
fn decode_image(bytes: &[u8]) -> Result<(u32, u32, Vec<u8>), String> {
    ...
    let expected = (w as usize).saturating_mul(h as usize).saturating_mul(4).saturating_add(8);
...
fn decode_png(data: &[u8]) -> Result<(u32, u32, Vec<u8>), String> {
    ...
    for row in 0..expected_rows {
        ...
        let pixel_end = (pixel_start + (width as usize).saturating_mul(bpp)).min(decompressed.len());
...
fn clamp_image(arr: &Array) -> Vec<u8> {
    ...
    let channels = if shape.len() == 3 { shape[0] as usize } else { 3 };
    let height = if shape.len() == 3 { shape[1] as usize } else { shape[0] as usize };
    let width = if shape.len() == 3 { shape[2] as usize } else { shape[1] as usize };
...
fn encode_png(rgba: &[u8], width: u32, height: u32) -> Result<Vec<u8>, String> {
    if rgba.len() < (width as usize).saturating_mul(height as usize).saturating_mul(3) {
    ...
    let mut raw_data = Vec::with_capacity(
        (height as usize).saturating_mul(row_len + 1),
    );
...
pub fn base64_encode(data: &[u8]) -> String {
    ...
    for chunk in data.chunks(3) {
        ...
        out.push(if chunk.len() > 1 { BASE64_CHARS[((triple >> 6) & 0x3F) as usize] } else { b'=' });
        out.push(if chunk.len() > 2 { BASE64_CHARS[(triple & 0x3F) as usize] } else { b'=' });
    }
