...
fn unsafe_reshape(arr: &Array, shape: &[i32]) -> Result<Array, String> {
    arr.reshape(shape).map_err(|e| format!("reshape {:?}: {:?}", shape, e))
}
...
fn timestep_embedding(t: &Array, dim: i32, max_period: f32) -> Result<Array, String> {
    ...
    let freqs: Vec<f32> = (0..half)
        ...
        .collect();
    let freqs_a = Array::from_slice(&freqs, &[1, half]).map_err(|_| "freqs".to_string())?;
...
impl TextToImageGenerator {
    ...
    pub fn generate(
        ...
        let latents = mlx_rs::random::normal::<f32>(
            &[1, n_patches, in_channels], None, None, None,
        )
        ...
        for i in 0..num_steps as usize {
            ...
            let dt_a = Array::from_slice(&[dt], &[1])
                .map_err(|_| "dt".to_string())?;
            let update = ops::multiply(&dt_a, &v_pred)
                .map_err(|e| format!("euler *: {:?}", e))?;
            x = ops::add(&x, &update)
                .map_err(|e| format!("euler +: {:?}", e))?;
        }
    ...
    fn encode_text(&self, input_ids: &[u32]) -> Result<(Array, Array), String> {
        ...
        for layer_idx in 0..cfg.num_hidden_layers {
            ...
            let q_b = tensor_by_name(&self.model, &format!("{}.self_attn.q_proj.bias", prefix)).ok();
            let k_b = tensor_by_name(&self.model, &format!("{}.self_attn.k_proj.bias", prefix)).ok();
            let v_b = tensor_by_name(&self.model, &format!("{}.self_attn.v_proj.bias", prefix)).ok();
            let o_b = tensor_by_name(&self.model, &format!("{}.self_attn.o_proj.bias", prefix)).ok();
            ...
            // Residual
            h = ops::add(&h, &attn_o).map_err(|e| format!("attn residual[{}]: {:?}", layer_idx, e))?;

            // MLP
            let post_ln_w = tensor_by_name(&self.model, &format!("{}.post_attention_layernorm.weight", prefix))?;
            ...
            h = ops::add(&h, &mlp_out).map_err(|e| format!("mlp residual[{}]: {:?}", layer_idx, e))?;
    ...
    fn flux_forward(
        ...
        for blk in 0..params.depth as usize {
            ...
            let (img_attn, txt_attn) = self.double_block_attn(
                &img, &txt_h, &p, &img_s1, &img_sc1, &img_s2, &img_sc2,
                n_heads, head_dim,
            )?;
            ...
            // MLP
            let (img_mlp, txt_mlp) = self.double_block_mlp(
                &img, &txt_h, &p, &img_s2, &img_sc2,
            )?;
        ...
        for blk in 0..params.depth_single as usize {
            ...
            let (attn_out, mlp_out) = self.single_block_fwd(&merged, &p, sh, sc, n_heads, head_dim)?;
            let gated = gate(&ops::add(&attn_out, &mlp_out)
                .map_err(|e| format!("attn+mlp[{}]: {:?}", blk, e))?, g)?;
    ...
    fn double_block_attn(
        ...
        let img_attn = unsafe_reshape(&attn_m.index((.., ..img_seq, ..)), &[1, img_seq, n_heads * head_dim])?;
    ...
    fn vae_decode(&self, latents: &Array) -> Result<Array, String> {
        ...
        let sf_a = Array::from_slice(&[cfg.scale_factor], &[1])
            .map_err(|_| "scale".to_string())?;
        let sh_a = Array::from_slice(&[cfg.shift_factor], &[1])
            .map_err(|_| "shift".to_string())?;
        let centred = ops::add(
        ...
        let one = Array::from_slice(&[1.0f32], &[1]).map_err(|_| "one".to_string())?;
        let half = Array::from_slice(&[0.5f32], &[1]).map_err(|_| "half".to_string())?;
