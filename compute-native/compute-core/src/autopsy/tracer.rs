...
use crate::autopsy::{AnomalyTrace, InferenceAnomaly, replay::ReplayResult};
...
impl AnomalyTracer {
    ...
    pub fn trace(&self, anomaly: &InferenceAnomaly) -> Result<AnomalyTrace, String> {
        ...
        let (weight_tensor, segment_file, source_hash) = weight_names
            ...
            .filter_map(|name| {
                ...
                Some((name.clone(), seg.filename.clone(), entry.source_sha256.clone()))
            })
            .next()
    ...
    pub fn replay_weight(&self, trace: &AnomalyTrace) -> Result<ReplayResult, String> {
        ...
        let segment_data = std::fs::read(&segment_path)
            .map_err(|e| format!("read segment file: {}", e))?;
    ...
    for i in 0..n {
        ...
        if offset + 4 <= bytes.len() {
            let val = f32::from_le_bytes(
                bytes[offset..offset + 4].try_into().unwrap_or([0u8; 4]),
            );
