...
fn write_mlpackage_inner(
    ...
    let model = proto::Model {
        ...
        description: Some(proto::ModelDescription {
            ...
            functions: vec![proto::FunctionDescription {
                ...
                input: build_feature_descriptions(
                    &program,
                    &description.function_name,
                    &description.inputs,
                ),
                output: build_feature_descriptions(
                    &program,
                    &description.function_name,
                    &description.outputs,
                ),
    ...
    fs::create_dir_all(&data_dir).map_err(|e| format!("mkdir {}: {}", data_dir.display(), e))?;
...
/// Build feature descriptions matching the MIL program's actual data types.
fn build_feature_descriptions(
    program: &mil_spec::Program,
    function_name: &str,
    entries: &[(String, Vec<i64>)],
) -> Vec<proto::FeatureDescription> {
    // Build a lookup map from the function inputs.
    let mut type_map: HashMap<&str, i32> = HashMap::new();
    if let Some(func) = program.functions.get(function_name) {
        for nvt in &func.inputs {
            if let Some(vt) = &nvt.r#type {
                if let Some(value_type::Type::TensorType(tt)) = &vt.r#type {
                    type_map.insert(nvt.name.as_str(), tt.data_type);
                }
            }
        }
        // Also scan block operation outputs for SSA values.
        for block in func.block_specializations.values() {
            for op in &block.operations {
                for out in &op.outputs {
                    if let Some(vt) = &out.r#type {
                        if let Some(value_type::Type::TensorType(tt)) = &vt.r#type {
                            type_map.entry(out.name.as_str()).or_insert(tt.data_type);
                        }
                    }
                }
            }
    ...
    let mil_to_array = |mil_dt: i32| -> i32 {
        match mil_dt {
    ...
    entries
        .iter()
        .map(|(name, shape)| {
            let data_type = type_map
                .get(name.as_str())
                .copied()
                .map(mil_to_array)
                .unwrap_or(proto::array_feature_type::ArrayDataType::Float32 as i32);
