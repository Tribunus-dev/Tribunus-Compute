...
mod tests {
    ...
    fn test_video_encoder_new_default() {
        ...
        let encoder = DummyFrameEncoder { patches: 256, proj_dim: 1024 };
        let video_enc = VideoEncoder::new(Box::new(encoder), &config);
        assert!(video_enc.is_ok());
        let video_enc = video_enc.unwrap();
        assert!(matches!(video_enc.temporal_agg, TemporalAggregation::AveragePool));
    ...
    fn test_encode_single_frame() {
        let config = VideoConfig::default();
        let dummy = DummyFrameEncoder { patches: 16, proj_dim: 32 };
