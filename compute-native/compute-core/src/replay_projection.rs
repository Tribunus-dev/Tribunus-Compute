...
impl ProjectionHarness {
    ...
    pub fn open(image_dir: &Path, layer_index: usize, family_name: &str) -> Result<Self, String> {
        let family = projection_identity::family_from_str(family_name)
            .ok_or_else(|| format!("unknown projection family: {}", family_name))?;

        let reader =
            CompiledImageReader::open(image_dir).map_err(|e| format!("open image: {}", e))?;

        let logical_hash = reader.manifest.image_hash.clone();
        let hidden_size = reader.manifest.architecture.hidden_size as i32;
        let root_hash =
            "3a0c6d47d42b42cba87d1be8a764a9717c9836e2748e4c0b3932ead6f645d8dd".to_string();

        let layer_plan = reader
            .manifest
            .execution_plan
            .layers
            .get(layer_index)
    ...
    fn replay(&self, samples: usize, warmups: usize, phase: &str) -> Vec<ReplaySample> {
        ...
        let w_elem_bytes = if w_elem_count > 0 {
            self.weight.nbytes() / w_elem_count
        } else {
            0
        };
        ...
        let run_one = |sample_index: usize, label: &str| -> ReplaySample {
            ...
            let (max_abs, max_rel, mean_abs, cosine, oracle_status) = if input_shape.len() > 0
                && input_shape[0] == 1
            {
            ...
            ReplaySample {
                ...
                m: if input_shape.len() > 0 {
                    input_shape[0]
                } else {
                    0
                },
                n: w_logical.get(0).copied().unwrap_or(0),
                k: if input_shape.len() > 1 {
                    input_shape[1]
                } else {
                    0
                },
                ...
                total_ns,
                mlx_active_before: if active_before > 0 {
                    Some(active_before)
                } else {
                    None
                },
                mlx_active_after: if active_after > 0 {
                    Some(active_after)
                } else {
                    None
                },
                ...
                mlx_cache_after: if cache_after > 0 {
                    Some(cache_after)
                } else {
                    None
                },
    ...
    pub fn replay_with_pipeline_warm(&self) -> Vec<ReplaySample> {
        ...
        fn make_owned_array(shape: &[i32], fill: f32) -> Array {
            Array::full::<f32>(shape, &Array::from_f32(fill)).expect("create owned array")
        }
    ...
    pub fn replay_with_page_touch(&self) -> Vec<ReplaySample> {
        ...
        fn volatile_page_touch<T>(slice: &[T]) -> (u64, u64, u64) {
            ...
            while off < byte_len {
                ...
                checksum =
                    checksum.wrapping_add(unsafe { std::ptr::read_volatile(ptr.add(off)) } as u64);
                off += 4096;
                pages += 1;
            }
    ...
    fn replay_one(&self, input_shape: &[i32], label: &str) -> ReplaySample {
        let input = Array::full::<f32>(input_shape, &Array::from_f32(0.5)).expect("create input");
        let ws = self.weight.shape();
        let w_logical = ws.iter().map(|d| *d as i32).collect::<Vec<_>>();
        let w_strides = self.weight.strides().to_vec();
        let w_elem_count: usize = ws.iter().map(|d| *d as usize).product();
    ...
    fn compare_oracle(
        ...
        let wf = match mlx_rs::ops::dequantize(weight, scale, bias, 64, 8) {
            ...
            Err(_) => {
                return (
                    ...
                    None,
                    None,
                    "unavailable (dequantize failed)".to_string(),
                )
            }
        };
        let wf_t = match mlx_rs::ops::transpose(&wf) {
            ...
            Err(_) => {
                return (
                    ...
                    None,
                    None,
                    "unavailable (transpose failed)".to_string(),
                )
            }
        };
        ...
        let oracle_arr = match oracle_input.matmul(&wf_t) {
            ...
            Err(_) => {
                return (
                    ...
                    None,
                    None,
                    "unavailable (matmul failed)".to_string(),
                )
            }
        };
        ...
        let mlx_out = match mlx_output.try_as_slice::<f32>() {
            ...
            Err(_) => {
                return (
                    ...
                    None,
                    None,
                    "unavailable (dtype mismatch)".to_string(),
                )
            }
        };
        let oracle_out = match oracle_arr.try_as_slice::<f32>() {
            ...
            Err(_) => {
                return (
                    ...
                    None,
                    None,
                    "unavailable (oracle dtype)".to_string(),
                )
            }
        };
        ...
        if oracle_out.len() != mlx_out.len() {
            return (
                ...
                None,
                format!(
                    "output length mismatch: oracle={}, mlx={}",
                    oracle_out.len(),
                    mlx_out.len()
                ),
            );
...
mod tests {
    ...
    fn synthetic_oracle_agreement() {
        ...
        let mlx_output =
            mlx_rs::ops::quantized_matmul(&input, &weight, &scale, &bias, true, group_size, bits)
        ...
        assert!(
            max_rel.unwrap() < 1e-2,
            "max_rel {} exceeds 1e-2 tolerance",
            max_rel.unwrap()
        );
        assert!(cosine.is_some(), "cosine should be Some, got {cosine:?}");
        assert!(
            (cosine.unwrap() - 1.0).abs() < 1e-4,
            "cosine {} not ~1.0",
            cosine.unwrap()
        );
