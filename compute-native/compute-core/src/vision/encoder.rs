...
impl VisionEncoder {
    ...
    pub fn encode(&self, image: &Array) -> Result<Array, String> {
        ...
        projected.eval().map_err(|e| {
            format!("vision encoder projection eval: {}", e)
        })?;

        let proj_dim = self.config.projection_dim as i32;
        // Flatten to [num_patches, projection_dim] removing batch dim.
        let projected_flat = projected.reshape(&[-1, proj_dim]).map_err(|e| {
            format!("reshape projected: {}", e)
        })?;
    ...
    fn extract_patches(&self, image: &Array, patch_size: i32, channels: i32) -> Result<Array, String> {
    ...
    fn run_encoder_layer(
        ...
        // Residual 2.
        let hidden = mlx_rs::ops::add(&hidden, &down)
            .map_err(|e| format!("ffn residual add: {:?}", e))?;
...
fn self_attention(
    ...
    let scores = mlx_rs::ops::matmul(&q_reshaped, &k_t)
        .map_err(|e| format!("attn scores: {:?}", e))?;
    let scores_scaled = mlx_rs::ops::multiply(
        &scores,
    ...
    // Attention @ V.
    let out = mlx_rs::ops::matmul(&attn, &v_reshaped)
        .map_err(|e| format!("attn output: {:?}", e))?;
