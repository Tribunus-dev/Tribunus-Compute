...
impl LoraAdapter {
            ...
            for (l, _layer_plan) in plan.layers.iter().enumerate() {
                ...
                let q = if is_target && target_modules.contains("q_proj") {
                    ...
                    if let Some(la) = lora_a_map.get(&key) {
                        let lora_out = matmul_lora(&normed, la, lora_b_map.get(&key).unwrap(), alpha, rank);
    ...
    pub fn merge(&self, model: &mut LoadedProfiledModel) -> Result<(), String> {
        ...
        for &layer_idx in &layers {
            ...
            for module in &self.target_modules {
                let key = format!("{}_{}_weight", layer_idx, module);
                let Some(lora_a) = self.lora_a.get(&key) else { continue };
    ...
    pub fn load(path: &str) -> Result<Self, String> {
        ...
        // Safetensors
        let st_path = dir.join("adapter_model.safetensors");
        let loaded = Array::load_safetensors(&st_path)
...
impl SharedWeightTable {
    ...
    pub fn forward_with_adapter(
        ...
        for (l, _layer_plan) in plan.layers.iter().enumerate() {
            ...
            // Q projection
            let q = matmul(&normed, &lw.q_proj_w, &lw.q_proj_s, &lw.q_proj_b);
            let q = apply_lora_if_needed(q, &normed, &adapter, &target_modules, is_target, l, "q_proj", alpha, rank);

            // K projection
            let k = matmul(&normed, &lw.k_proj_w, &lw.k_proj_s, &lw.k_proj_b);
            let k = apply_lora_if_needed(k, &normed, &adapter, &target_modules, is_target, l, "k_proj", alpha, rank);

            // V projection
            let v = matmul(&normed, &lw.v_proj_w, &lw.v_proj_s, &lw.v_proj_b);
            let v = apply_lora_if_needed(v, &normed, &adapter, &target_modules, is_target, l, "v_proj", alpha, rank);
...
) -> (&'a mut std::sync::Arc<Array>, &'a mut std::sync::Arc<Array>, &'a mut std::sync::Arc<Array>) {
    match module {
        ...
        "gate_proj" => (&mut lw.gate_proj_w, &mut lw.gate_proj_s, &mut lw.gate_proj_b),
        "up_proj" => (&mut lw.up_proj_w, &mut lw.up_proj_s, &mut lw.up_proj_b),
        "down_proj" => (&mut lw.down_proj_w, &mut lw.down_proj_s, &mut lw.down_proj_b),
