...
use super::manifest::{
    mlx_active_memory_bytes, mlx_cache_memory_bytes,
        ...
        for entry in &self.manifest.tensor_table {
            ...
            if let Some(quantization) = &entry.quantization {
                ...
                let biases_entry = self
                    ...
                    .ok_or_else(|| {
                        crate::Error::from_reason(format!(
                            "missing bias tensor for {}",
                            entry.name
                        ))
                    })?;

                let w_handle =
                    *self.persistent_handles.get(&entry.name).ok_or_else(|| {
                        crate::Error::from_reason(format!(
                            "missing persistent handle: {}",
                            entry.name
                        ))
                    })?;
                let s_handle = *self
                ...
                self.quantized_bindings
                    .insert(entry.name.clone(), binding);
            }
        }
        Ok(())
    }
    ...
    pub fn run_six_layer_prefix(&mut self) -> crate::Result<Array> {
        if self.released {
            return Err(crate::Error::from_reason(
                "image runtime already released",
            ));
        }
        ...
        let layer_count = usize::min(
            ...
            usize::min(
                ...
                arch.num_hidden_layers as usize,
            ),
        );
        ...
        let (emb_w, emb_s, emb_b) = {
            ...
            let emb_w = reg
                ...
                .cloned()
                .ok_or_else(|| {
                    crate::Error::from_reason("embed weight handle invalid")
                })?;
            let emb_s = reg
                .get(
                    *self
                ...
                .cloned()
                .ok_or_else(|| {
                    crate::Error::from_reason("embed scales handle invalid")
                })?;
            let emb_b = reg
                .get(
                    *self
                ...
                .cloned()
                .ok_or_else(|| {
                    crate::Error::from_reason("embed biases handle invalid")
                })?;
            (emb_w, emb_s, emb_b)
        };
        ...
        let mut hidden =
            crate::primitives::quantized_embedding_lookup(&tok, &emb_w, &emb_s, &emb_b)
                .map_err(|e| {
                    crate::Error::from_reason(format!("embed lookup: {:?}", e))
                })?
                .multiply(&Array::from_f32((arch.hidden_size as f32).sqrt()))
                .map_err(|e| {
                    crate::Error::from_reason(format!("embed scale: {:?}", e))
                })?;
        ...
        for layer in 0..layer_count {
            ...
            let layer_map =
                self.build_layer_arrays_from_lease(layer as u32, &lease)?;
            ...
            let attn_norm =
                get_tensor(&format!("{}.input_layernorm.weight", base))?;
            let ffn_norm =
                get_tensor(&format!(
        ...
        let fn_w = {
            ...
            .cloned()
            .ok_or_else(|| {
                crate::Error::from_reason("norm weight handle invalid")
            })?
        };
        let final_hidden = crate::primitives::rms_norm(&hidden, &fn_w, 1e-6)
            .map_err(|e| {
                crate::Error::from_reason(format!("final norm: {:?}", e))
            })?;
        ...
        let out = {
            ...
            let ew = reg
                    ...
                    *self
                        ...
                        .ok_or_else(|| {
                            crate::Error::from_reason(
                                "embed weight gone before lm_head",
                            )
                        })?,
                )
                .cloned()
                .ok_or_else(|| {
                    crate::Error::from_reason(
                        "embed weight handle invalid at lm_head",
                    )
                })?;
            let es = reg
                .get(
                    *self
                        ...
                        .ok_or_else(|| {
                            crate::Error::from_reason(
                                "embed scales gone before lm_head",
                            )
                        })?,
                )
                .cloned()
                .ok_or_else(|| {
                    crate::Error::from_reason(
                        "embed scales handle invalid at lm_head",
                    )
                })?;
            let eb = reg
                .get(
                    *self
                        ...
                        .ok_or_else(|| {
                            crate::Error::from_reason(
                                "embed biases gone before lm_head",
                            )
                        })?,
                )
                .cloned()
    ...
    pub fn run_full_model(&mut self, token_ids: &[i32]) -> crate::Result<u32> {
        ...
        let (emb_w, emb_s, emb_b) = {
            ...
            let w = reg
                ...
                .cloned()
                .ok_or_else(|| {
                    crate::Error::from_reason("embed weight invalid")
                })?;
            let s = reg
                .get(
                    *self
                ...
                .cloned()
                .ok_or_else(|| {
                    crate::Error::from_reason("embed scales invalid")
                })?;
            let b = reg
                .get(
                    *self
                ...
                .cloned()
                .ok_or_else(|| {
                    crate::Error::from_reason("embed biases invalid")
                })?;
            (w, s, b)
        };
        ...
        hidden.eval().map_err(|e| {
            crate::Error::from_reason(format!("prologue eval: {:?}", e))
        })?;
        ...
        .map_err(|e| {
            crate::Error::from_reason(format!("rope local: {:?}", e))
        })?;
        let full_rope = arch.rope_global.as_ref().unwrap_or(&arch.rope_local);
        let (full_cos, full_sin) = crate::primitives::rope_freqs(
            arch.global_head_dim.unwrap_or(arch.head_dim),
        ...
        .map_err(|e| {
            crate::Error::from_reason(format!("rope global: {:?}", e))
        })?;
        ...
        for layer_plan in &plan.layers {
            let is_sliding =
                layer_plan.attention_kind == "sliding_attention";
        ...
        for layer_plan in &plan.layers {
            ...
            let lease = self.activate_layer(l).map_err(|e| {
                crate::Error::from_reason(format!(
                    "activate layer {}: {}",
                    l, e
                ))
            ...
            let layer_map =
                self.build_layer_arrays_from_lease(l, &lease)?;
            ...
            let attn_norm =
                get_tensor(&format!("{}.input_layernorm.weight", base))?;
            let ffn_norm = get_tensor(&format!(
                "{}.post_attention_layernorm.weight",
                base
            ))?;
            let (qw, qs, qb) = (
            ...
            .map_err(|e| {
                crate::Error::from_reason(format!("layer {}: {:?}", l, e))
            })?;
        ...
        let fn_w = {
            ...
            .cloned()
            .ok_or_else(|| {
                crate::Error::from_reason("norm weight invalid")
            })?
        };
        ...
        .map_err(|e| {
            crate::Error::from_reason(format!("epilogue: {:?}", e))
        })?;

        epi.selected_token.eval().map_err(|e| {
            crate::Error::from_reason(format!("epilogue eval: {:?}", e))
        })?;
        let token_id = epi
            .selected_token
            .try_as_slice::<u32>()
