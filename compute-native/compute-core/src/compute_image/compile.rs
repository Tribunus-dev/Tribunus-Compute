...
use super::manifest::{
    CompilationAuthority, CompileReceipt, CompiledImage, CompiledImageReader, ImageBuilder,
    AliasEntry, Manifest, ManifestVerification, NativeCapabilityReport, QuantizationDesc, ResidencyPlan, Segment,
    SegmentKind, SegmentReceipt, ShardHash, SourceIdentity, StageProfile, StorageBackend,
...
pub fn compile_with_authority(
    ...
    let quantize_mode = quantize_mode.or_else(|| {
        CompileQuantMode::from_name(target.recommended_quant())
    });
    ...
    compile_unchecked(source_dir, output_dir, skip_validation, quantize_mode).map(
        |mut compiled| {
...
fn verify_fixture_ceiling(source_dir: &str) -> crate::Result<()> {
    ...
    if total_bytes > max_fixture_bytes {
        return Err(crate::Error::new(
            crate::Status::GenericFailure,
            format!(
                "TestFixture source ceiling: {max_fixture_bytes} bytes, found {total_bytes}"
            ),
        ));
    }
    Ok(())
    ...
    for shard_path in shard_paths {
        ...
        for (name, info) in entries {
            ...
            let view = safetensors
                .tensor(&name)
                .map_err(|e| {
                    crate::Error::from_reason(format!("tensor {}: {:?}", name, e))
                })?;
...
fn download_hf_file(
    ...
    let api = builder.with_token(token).build().map_err(|e| {
        crate::Error::from_reason(format!("HF API init: {e}"))
    })?;
    ...
    let cached_path = model.get(filename).map_err(|e| {
        crate::Error::from_reason(format!(
            "hf download {hub_id}/{filename}@{revision}: {e}"
        ))
    })?;

    // Hardlink or copy from HF cache to our dest_dir
    fs::hard_link(&cached_path, &dest).or_else(|_| {
        fs::copy(&cached_path, &dest).map(|_| ())
    }).map_err(|e| {
        crate::Error::from_reason(format!(
            "link/copy {} -> {}: {e}",
            cached_path.display(),
...
pub fn download_hf_model(
    ...
    let shard_list = match fetch_shard_list(hub_id, revision, dest_dir, hf_token) {
        ...
        _ => {
            let _ =
                download_hf_file(hub_id, "model.safetensors", revision, dest_dir, hf_token);
            return Ok(());
        }
    };
...
fn compile_vision_encoder_tensors(
    ...
    if emitted_ids
        .keys()
        .any(|k| k.starts_with("vision_encoder."))
    {
        return Ok(());
    }
...
fn compile_audio_encoder_tensors(
    ...
    if emitted_ids
        .keys()
        .any(|k| k.starts_with("audio_encoder."))
    {
        return Ok(());
    }
...
fn compute_manifest_hash(manifest: &Manifest) -> String {
    ...
    let bytes =
        serde_json::to_vec(&fingerprint).expect("manifest fingerprint serialization");
    sha256_bytes(&bytes)
}
        ...
        .filter_map(|entry| {
            loaded.source_tensors.get(&entry.name).map(|source_tensor| {
                ...
                TensorProvenance {
                    ...
                    preserved_byte_for_byte: source_tensor.source_sha256
                        == emitted_sha256,
                }
            })
        })
...
fn embed_metallib(
    ...
    let found = candidates.iter().find(|p| p.exists() && p.is_file()).cloned();
...
pub(crate) fn compile_sequential(
    ...
    for binding in &loaded.spec.global_tensors {
        let id =
            emit_binding_set(&mut builder, &loaded.source_tensors, binding, None)?;
        emitted_ids.insert(binding.name.clone(), id);
    }

    if loaded.namespace.lm_head_aliased {
        let embed_name =
            format!("{}.embed_tokens.weight", loaded.namespace.root);
        let physical_id = emitted_ids.get(&embed_name).copied().ok_or_else(|| {
    ...
    embed_metallib(&mut builder, source_dir, output_dir, quantize_mode, &loaded.arch)?;
...
pub fn compile_differential(
    ...
    let prev_manifest: Manifest = serde_json::from_str(&prev_manifest_text).map_err(|e| {
    ...
    let (new_segments, new_payloads, partial_manifest) =
        builder.flush_and_collect_segments();
    ...
    let mut combined_tensors: Vec<TensorEntry> = Vec::with_capacity(
        prev_manifest.tensor_table.len() + partial_manifest.tensor_table.len(),
    );
    ...
    let total_source_bytes: u64 = loaded
        ...
        .sum();
    let total_emitted_bytes: u64 = combined_manifest
        .segments
        .iter()
        .map(|s| s.byte_size)
        .sum();
...
pub fn run_diagnostics(image_dir: &Path) -> crate::Result<DiagnosticReport> {
    ...
    report.global.inf_layers = report
        ...
        .count();
    report.global.total_runtime_ms =
        report.layers.iter().map(|l| l.elapsed_ms).sum();
