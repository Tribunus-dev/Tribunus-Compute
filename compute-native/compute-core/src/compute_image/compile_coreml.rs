...
pub fn compile_subgraph(
    ...
    if coreml_ops.is_empty() {
        return Err("no ops to compile".to_string());
    }

    // Phase 1: Build MIL program
    let program = build_mil_program(name, coreml_ops, input_shapes)?;

    // Phase 2: Write .mlpackage
    let mlpackage_dir = output_dir.join(format!("{}.mlpackage", name));
    let _ = std::fs::create_dir_all(&mlpackage_dir);

    // Determine inputs/outputs from the MIL program.
    ...
    // For now use a generic "output" -- the MilBuilder tracks these internally
    // but doesn't expose the final SSA name directly.  The caller is expected
    // to supply it via input_shapes["output"].
    let output_name = input_shapes
        .keys()
        .find(|k| *k == "output")
        .map(|_| "output".to_string())
        .unwrap_or_else(|| format!("{}_output", name));
    ...
    let meta = ModelMeta {
        ...
        inputs: vec![
            ("x".into(), vec![1, hidden_dim]),
            ("residual".into(), vec![1, hidden_dim]),
        ],
        outputs: vec![(output_name, vec![1, hidden_dim])],
    };

    let written_path =
        mlpackage::write_mlpackage(program, &mlpackage_dir, &meta)
