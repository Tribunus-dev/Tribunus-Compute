...
use super::manifest::{
    CompiledImageReader, ManifestVerification, StorageBackend, mlx_peak_memory_bytes,
};
use serde::Serialize;
use std::path::Path;
...
pub fn publish_image(staging: &Path, destination: &Path) -> crate::Result<()> {
    ...
    std::fs::write(&publishing_marker, b"").map_err(|e| {
        crate::Error::from_reason(format!("write .publishing: {}", e))
    })?;
