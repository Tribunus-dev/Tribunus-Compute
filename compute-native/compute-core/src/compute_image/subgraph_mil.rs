...
fn op_composite_silu(builder: MilBuilder, input: &str) -> (MilBuilder, String) {
    ...
    let builder = builder.mul(input, &sig_name);
    // mul output from MilBuilder: fresh_name("mul") → "mul_{counter-1}"
    // We need to track this. Let's use operation() directly instead.
...
pub fn build_matmul_mil(
    ...
    let builder = MilBuilder::new("main")
        ...
        .const_f32(weight_name, &[], &[k as i64, n as i64])
        .matmul(input_name, &format!("{}_0", weight_name));
    let builder = builder.output("matmul_1");
    builder.build().map_err(|e| format!("matmul MIL build failed: {e}"))
}
...
pub fn build_mlp_block_mil(
    ...
    builder.build().map_err(|e| format!("MLP block MIL build failed: {e}"))
}
...
pub fn build_rmsnorm_qkv_mil(
    ...
    let builder = builder.const_f32("w_rms", &[], &[1, h]);
    ...
    builder.build().map_err(|e| format!("RMSNorm+QKV MIL build failed: {e}"))
}
...
pub fn build_output_proj_mil(
    ...
    let builder = MilBuilder::new("main")
        .input(input_name, mil_spec::DataType::Float32, &[1, h])
        .const_f32("w_lm_head", &[], &[h, v])
        .matmul(input_name, "w_lm_head_0")
        .output("matmul_1");

    builder.build().map_err(|e| format!("output proj MIL build failed: {e}"))
}
...
pub fn build_ffn_output_mil(
    ...
    builder.build().map_err(|e| format!("FFN+output MIL build failed: {e}"))
}
...
pub fn build_qkv_bundle_mil(
    ...
    builder.build().map_err(|e| format!("QKV bundle MIL build failed: {e}"))
}
...
mod tests {
    ...
    fn build_mlp_block_acceptance() {
        ...
        assert!(block.operations.len() >= 6, "expected >=6 ops, got {}", block.operations.len());
