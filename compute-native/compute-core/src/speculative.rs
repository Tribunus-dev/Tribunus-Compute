...
pub fn resample(
    ...
    match strategy {
        ...
        SampleStrategy::Temperature(temp) => {
            ...
            tokens.iter().copied().zip(probs.iter().map(|lp| lp * inv_temp)).collect()
        }
        SampleStrategy::TopK(_k) => {
            tokens.iter().copied().zip(probs.iter().copied()).collect()
        }
        SampleStrategy::TopP(_p) => {
            tokens.iter().copied().zip(probs.iter().copied()).collect()
        }
        SampleStrategy::Contrastive { alpha, beta } => {
            tokens.iter().copied().zip(probs.iter().map(|lp| {
                lp * (1.0 - *alpha) - *beta
            })).collect()
        }
        SampleStrategy::Mirostat { tau, learn_rate } => {
            tokens.iter().copied().zip(probs.iter().map(|lp| {
        ...
        SampleStrategy::Typical(p) => {
            tokens.iter().copied().zip(probs.iter().map(|lp| lp * *p)).collect()
        }
        ...
        SampleStrategy::Eta(_eta) => {
            tokens.iter().copied().zip(probs.iter().map(|lp| lp * 0.9)).collect()
        }
        SampleStrategy::LocallyTypical { k, tau } => {
            tokens.iter().copied().zip(probs.iter().map(|lp| {
                lp * (*tau as f32) / (*k as f32).max(1.0)
            })).collect()
        }
...
impl MultiSpecDraftModel {
    ...
    pub fn default_strategies() -> [SampleStrategy; 16] {
        [
            ...
            SampleStrategy::Contrastive { alpha: 0.5, beta: 0.1 },
            SampleStrategy::TopK(40),
            SampleStrategy::TopP(0.9),
            SampleStrategy::Mirostat { tau: 2.0, learn_rate: 0.1 },
    ...
    pub fn ultra_strategies() -> [SampleStrategy; 32] {
        [
            ...
            SampleStrategy::Contrastive { alpha: 0.3, beta: 0.05 },
            SampleStrategy::Contrastive { alpha: 0.7, beta: 0.15 },
            SampleStrategy::Contrastive { alpha: 0.9, beta: 0.2 },
...
impl DraftModel for MultiSpecDraftModel {
    ...
    fn speculate(
        ...
        // Run all drafts
        let all_results = self.multi_core.speculate_all(prefix, n_tokens, &self.strategies)?;
...
impl MultiSpecDraftModel {
    ...
    pub fn get_draft_probs(&self, draft_tokens: &[Vec<u32>]) -> Result<mlx_rs::Array, String> {
        ...
        let seq_len = if num_drafts > 0 { draft_tokens[0].len() } else { 0 };
