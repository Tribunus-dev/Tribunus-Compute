...
impl CompressedKvSlot {
    ...
    pub fn allocated_bytes(&self) -> u64 {
        let data = self.compressed_keys.len() + self.compressed_values.len()
            + self.qjl_correction.as_ref().map_or(0, |c| c.residual_bits.len());
...
pub fn evict_page_to_disk(page: &TiersPage) -> Result<String, String> {
    ...
    let compressed = page.l3_data.as_ref().ok_or_else(|| {
        "evict_page_to_disk: page has no l3_data".to_string()
    })?;

    std::fs::write(&filename, compressed)
        .map_err(|e| format!("write KV page: {}", e))?;
...
pub fn load_page_from_disk(filename: &str) -> Result<Vec<u8>, String> {
    let data = std::fs::read(filename)
        .map_err(|e| format!("read KV page: {}", e))?;
    ...
    pub fn allocated_bytes(&self) -> u64 {
        ...
        match self.current_tier {
            KVCacheTier::L1AneSram => {
                self.l1_data.as_ref().map_or(0, |d| d.len() as u64 * 4)
                    + overhead
            }
...
impl PageMigrationService {
    ...
    fn promote_to_l1(&self, page: &mut TiersPage) -> Result<(), String> {
        match page.current_tier {
            KVCacheTier::L3DramHeap => {
                ...
                let l3_bytes = page.l3_data.as_ref().ok_or_else(|| {
            ...
            KVCacheTier::L2Iosurface => {
                ...
                let _handle = page.l2_handle.as_ref().ok_or_else(|| {
                    "promote_to_l1: L2 page has no l2_handle".to_string()
                })?;
    ...
    fn demote_to_l3(&self, page: &mut TiersPage) -> Result<(), String> {
        match page.current_tier {
            KVCacheTier::L1AneSram => {
                ...
                let fp16 = page.l1_data.as_ref().ok_or_else(|| {
    ...
    pub fn prefetch_predicted(&mut self) -> Result<usize, String> {
        ...
        let hot_tokens: Vec<u32> = self.pages.iter()
            .filter(|p| {
        ...
        for &hot_start in &hot_tokens {
            ...
            for adj in [hot_start.saturating_sub(256), hot_start + 256] {
                if let Some(page) = self.pages.iter_mut().find(|p| {
                    p.current_tier == KVCacheTier::L4Disk && p.contains_token(adj)
                }) {
