...
impl OwnedBuffer {
    ...
    pub fn new(size: usize) -> Self {
        if size == 0 {
            ...
            assert!(
                !ptr.is_null(),
                "OwnedBuffer::new(0) allocation failed (OOM)"
            );
            Self {
                ptr,
                len: 0,
            ...
            assert!(
                !ptr.is_null(),
                "OwnedBuffer::new({size}) allocation failed (OOM)"
            );
            Self {
                ptr,
                len: size,
...
impl Drop for DeleterContext {
    fn drop(&mut self) {
        let _ = self.storage.byte_len();
    }
}
...
mod tests {
    ...
    fn test_external_array_round_trip() {
        ...
        eprintln!(
            "[no-copy] external array round trip: PASS (deleter_count={})",
            deleter_count()
        );
    }
    ...
    fn test_multiple_views_into_one_segment() {
        ...
        let segment: Arc<MappedSegment> =
            Arc::new(unsafe { MappedSegment::from_parts(raw_buf.as_ptr(), byte_len) });
        ...
        let arr1 = unsafe {
            new_external_array(
                storage1 as Arc<dyn ExternalStorage + Send + Sync>,
                shape,
                Dtype::Float32,
            )
        }
        ...
        let arr2 = unsafe {
            new_external_array(
                storage2 as Arc<dyn ExternalStorage + Send + Sync>,
                shape,
                Dtype::Float32,
            )
        }
    ...
    fn test_deleter_exactly_once() {
        ...
        let n: usize = (shape[0] * shape[1]) as usize;
        let byte_len = n * 4;
        let storage: Arc<dyn ExternalStorage + Send + Sync> = Arc::new(OwnedBuffer::new(byte_len));
    ...
    fn test_multiple_segment_views() {
        ...
        deleter_count_at_least(
            baseline,
            2,
            "deleter must fire at least twice for two arrays",
        );
    ...
    fn test_cloned_array_preserves_mapping() {
        ...
        deleter_count_at_least(
            baseline,
            1,
            "deleter must fire at least once for cloned arrays",
        );
    }
    ...
    fn test_static_storage_survives_forced_drop() {
        ...
        let static_storage: Arc<dyn ExternalStorage + Send + Sync> =
            Arc::new(unsafe { StaticStorage::new(TEST_STATIC_DATA.as_ptr(), byte_len) });

        let arr = unsafe { new_external_array(static_storage, shape, Dtype::Uint8) }
        ...
        assert_eq!(TEST_STATIC_DATA[0], 0x00, "static data[0] unchanged");
        assert_eq!(TEST_STATIC_DATA[15], 0x0F, "static data[15] unchanged");
        assert_eq!(TEST_STATIC_DATA[31], 0x1F, "static data[31] unchanged");
    }
}
