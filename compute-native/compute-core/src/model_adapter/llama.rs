...
impl ModelFamilyAdapter for LlamaAdapter {
    ...
    fn detect(&self, config: &Value, tensor_names: &[String]) -> bool {
        config.get("model_type").and_then(|v| v.as_str()) == Some("llama")
            && tensor_names.iter().any(|n| n.contains(".self_attn.q_proj.weight"))
    }
    ...
    fn normalize(&self, source: &SourceModel) -> Result<CanonicalModel, NormalizationReport> {
        ...
        // Read rope_scaling from config if present.
        let rope_type = cfg.get("rope_scaling")
