...
use super::{CanonicalModel, CanonicalRole, ModelFamilyAdapter, NormalizationReport, SourceModel, TensorData};
...
impl ModelFamilyAdapter for PhiAdapter {
    ...
    fn detect(&self, config: &Value, tensor_names: &[String]) -> bool {
        let type_match = config.get("model_type")
            .and_then(|v| v.as_str())
        ...
        let has_q_proj = tensor_names.iter().any(|n| n.contains(".self_attn.q_proj.weight"));
    ...
    fn normalize(&self, source: &SourceModel) -> Result<CanonicalModel, NormalizationReport> {
        ...
        let mt = cfg.get("model_type").and_then(|v| v.as_str()).unwrap_or("phi").to_string();
        ...
        let arch = TextArchitecture {
            ...
            model_type: mt,
            layer_types: (0..n_layers).map(|_| AttentionKind::FullAttention).collect(),
            rope_local: RopeSpec {
        ...
        let has_model_ns = source.tensors.contains_key("model.embed_tokens.weight")
            ...
            || source.tensors.keys().any(|k| k.starts_with("model.layers."));
        ...
        if let Some(t) = source.tensors.get(&embed_key).or_else(|| source.tensors.get(&wte_key)) {
            tensors.insert(CanonicalRole::Embedding, TensorData {
                dtype: t.0.clone(), shape: t.1.clone(), data: t.2.clone(),
            });
        } else {
        ...
        if let Some(t) = source.tensors.get(&norm_key).or_else(|| source.tensors.get(&ln_f_key)) {
            tensors.insert(CanonicalRole::FinalNorm, TensorData {
                dtype: t.0.clone(), shape: t.1.clone(), data: t.2.clone(),
            });
        } else {
        ...
        if let Some(t) = source.tensors.get(&lm_head_key) {
            tensors.insert(CanonicalRole::LmHead, TensorData {
                dtype: t.0.clone(), shape: t.1.clone(), data: t.2.clone(),
            });
        }
        ...
        for i in 0..n_layers {
            ...
            match source.tensors.get(&name) {
                Some(t) => {
                    tensors.insert(CanonicalRole::AttnNorm(i), TensorData {
                        dtype: t.0.clone(), shape: t.1.clone(), data: t.2.clone(),
                    });
                }
                None => {
            ...
            for proj in &["q", "k", "v", "o"] {
                ...
                match source.tensors.get(&name) {
                    Some(t) => {
                        tensors.insert(role, TensorData {
                            dtype: t.0.clone(), shape: t.1.clone(), data: t.2.clone(),
                        });
                    }
                    None => {
            ...
            match source.tensors.get(&name) {
                Some(t) => {
                    tensors.insert(CanonicalRole::MlpNorm(i), TensorData {
                        dtype: t.0.clone(), shape: t.1.clone(), data: t.2.clone(),
                    });
                }
                None => {
