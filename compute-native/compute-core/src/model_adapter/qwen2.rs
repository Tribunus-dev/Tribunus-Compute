...
/// Qwen2 model-family adapter.
#[derive(Debug, Clone, Copy)]
pub struct Qwen2Adapter;

impl ModelFamilyAdapter for Qwen2Adapter {
    ...
    fn detect(&self, config: &Value, tensor_names: &[String]) -> bool {
        config.get("model_type").and_then(|v| v.as_str()) == Some("qwen2")
    ...
    fn normalize(&self, source: &SourceModel) -> Result<CanonicalModel, NormalizationReport> {
        ...
        let arch = TextArchitecture {
            ...
            layer_types: (0..n_layers).map(|_| AttentionKind::SlidingAttention).collect(),
            rope_local: RopeSpec {
