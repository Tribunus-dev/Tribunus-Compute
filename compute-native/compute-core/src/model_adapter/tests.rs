...
fn fill_standard_tensors(
    ...
    let vo_dim = head_dim * num_kv_heads; // V/O see KV heads

    map.insert(
        "model.embed_tokens.weight".into(),
        ("F32".into(), vec![vocab_size, hs], vec![0u8; (vocab_size * hs * 4) as usize]),
    );
    ...
    for i in 0..num_layers {
        ...
        for proj in &["q", "k", "v", "o"] {
            ...
            map.insert(
                name,
                ("F32".into(), vec![hs, hs], vec![0u8; (hs * hs * 4) as usize]),
            );
        }
        // MLP
        for proj in &["gate", "up", "down"] {
            ...
            map.insert(
                name,
                ("F32".into(), vec![out_d, in_d], vec![0u8; (out_d * in_d * 4) as usize]),
            );
        }
    }
...
fn test_registry_rejects_unknown() {
    ...
    let err = registry.select(&config, &tensor_names).unwrap_err();
    assert!(
...
fn test_qwen2_normalize_success() {
    ...
    assert!(result.is_ok(), "qwen2 normalize succeeded: {:?}", result.err());
}
...
fn test_llama_normalize_success() {
    ...
    let result = adapter.normalize(&source);
    assert!(result.is_ok(), "llama normalize succeeded: {:?}", result.err());
}
...
fn test_mistral_normalize_success() {
    ...
    let result = adapter.normalize(&source);
    assert!(result.is_ok(), "mistral normalize succeeded: {:?}", result.err());
}
...
fn test_gemma_normalize_success() {
    ...
    let result = adapter.normalize(&source);
    assert!(result.is_ok(), "gemma normalize succeeded: {:?}", result.err());
}
...
fn test_phi_normalize_success() {
    ...
    let result = adapter.normalize(&source);
    assert!(result.is_ok(), "phi normalize succeeded: {:?}", result.err());
}
...
fn test_fixture_generation_end_to_end() {
    ...
    for name in &expected_names {
        let view = loaded.tensor(name).unwrap_or_else(|_| panic!("tensor {name} accessible"));
    ...
    let loaded_names: Vec<&str> = loaded.names();
    assert_eq!(loaded_names.len(), expected_names.len(), "tensor count matches");
    for name in &expected_names {
        assert!(
