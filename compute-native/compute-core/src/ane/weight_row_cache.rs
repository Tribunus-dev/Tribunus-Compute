...
impl WeightRowCache {
    ...
    fn f16_to_f32(h: u16) -> f32 {
        ...
        if exp == 0 {
            ...
            let mant32 = if mant == 0 { 0 } else { (mant << 13) | 0x7F_FFFF };
    ...
    fn load_row(&mut self, token_id: u32, row_data_f32: &[f32]) -> Result<(), String> {
        ...
        unsafe {
            ...
            for i in 0..hs {
                ptr.add(row_offset + i).write(Self::f32_to_f16(row_data_f32[i]));
            }
        }
    ...
    fn extract_row_from_lm_head(lm_head: &Array, token_id: u32, hidden_size: u32) -> Result<Vec<f32>, String> {
        ...
        row_flat.eval().map_err(|e| {
            format!("WeightRowCache: eval lm_head row: {:?}", e)
        })?;

        let row_f32: Vec<f32> = row_flat
            .try_as_slice::<f32>()
            .map_err(|e| {
                format!("WeightRowCache: read lm_head row as f32: {:?}", e)
            })?
            .to_vec();
    ...
    pub fn hybrid_lm_head(
        ...
        self.row_arena.lock().map_err(|e| format!("arena lock: {}", e))?;
        ...
        self.row_arena.unlock().map_err(|e| format!("arena unlock: {}", e))?;
