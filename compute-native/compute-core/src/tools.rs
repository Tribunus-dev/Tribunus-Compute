...
pub fn parse_and_repair(
    ...
    ToolCallResult::Unrepairable(
        "could not parse any JSON from model output".into(),
    )
}
...
pub fn validate_and_fix(
    ...
    if let Some(args) = val.get("arguments") {
        let reparsed = args.as_str().and_then(|s| {
...
pub fn fuzzy_match_function_name(
    ...
    let threshold = max(
        1,
        (tool.name.len() as f64 * 0.4).ceil() as usize,
    );
...
fn fix_single_value(
    ...
    match (expected_type, val) {
        ...
        ("string", serde_json::Value::Number(n)) => {
            Some(serde_json::json!(n.to_string()))
        }
        ("string", serde_json::Value::Bool(b)) => {
            Some(serde_json::json!(b.to_string()))
        }

        // Number/integer but got boolean
        ("number", serde_json::Value::Bool(true))
        | ("integer", serde_json::Value::Bool(true)) => {
            Some(serde_json::json!(1))
        }
        ("number", serde_json::Value::Bool(false))
        | ("integer", serde_json::Value::Bool(false)) => {
...
mod tests {
    ...
    fn test_fill_missing_required() {
        ...
        match parse_and_repair(raw, &tool) {
            ToolCallResult::Repaired(call, fixes) => {
                ...
                let args = call.arguments.get("arguments")
                    .and_then(|a| a.as_object())
                    .cloned()
                    .unwrap_or_default();
    ...
    fn test_type_mismatch_string_to_number() {
        ...
        let raw =
            r#"{"name": "calculate", "arguments": {"value": "42"}}"#;
        match parse_and_repair(raw, &tool) {
