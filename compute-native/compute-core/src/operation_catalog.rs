...
mod tests {
    use super::*;
    use crate::config::{EpiloguePlan, LayerPlan, ModelExecutionPlan, ProloguePlan};
    ...
    fn catalog_globals() {
        ...
        let globals: Vec<&OperationRecord> = catalog
            .records
            .iter()
            .filter(|r| r.layer_index.is_none())
            .collect();
    ...
    fn catalog_no_softcap() {
        ...
        assert!(catalog
            .records
            .iter()
            .all(|r| r.kind != OperationKind::Softcap));
    }
}
