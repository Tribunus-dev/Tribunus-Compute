    ...
    fn emit_matmul(
        ...
        Ok(StagedEmission {
            ...
            inventory: OpInventoryEntry {
                ...
                input_bindings: vec![("x".into(), a_ref.clone()), ("y".into(), b_ref.clone())],
                output_bindings: vec![(ssa, value_ref.clone())],
                output_types: vec![vt],
                opset: ctx.target.opset_identifier().into(),
                legal: true,
            ...
            _ => {
                return Err(CoreMlLoweringError::new(&ctx.region_identity).with_fatal(
                    LoweringDiagnostic::ConstraintViolation {
                        op_id: op.op_id,
                        constraint: "op_attrs".to_string(),
                        detail: "expected Transpose".into(),
                    },
                ))
            }
        };
        ...
        for (i, input_id) in op.inputs.iter().enumerate() {
            if !ctx.tensor_meta.contains_key(input_id) {
                diags.push(LoweringDiagnostic::ConstraintViolation {
                    ...
                    detail: format!(
                        "input[{}] tensor {:?} not registered in tensor_meta",
                        i, input_id
                    ),
                });
            }
        ...
        for (i, output_id) in op.outputs.iter().enumerate() {
            if !ctx.tensor_meta.contains_key(output_id) {
                diags.push(LoweringDiagnostic::ConstraintViolation {
                    ...
                    detail: format!(
                        "output[{}] tensor {:?} not registered in tensor_meta",
                        i, output_id
                    ),
                });
            }
...
impl OpRegistry {
    ...
    fn register(&mut self, opcode: Opcode, impl_: OpImpl) {
        self.emitters.insert(opcode, impl_);
    }

    fn get(&self, opcode: Opcode) -> Option<&OpImpl> {
...
impl CoreMlLowering {
    ...
    pub fn lower_with_ops(
        ...
        let pkg_path = mlpackage::write_mlpackage(mil_artifact.program, pkg_dir.path(), &meta)
            .map_err(|e| format!("write_mlpackage: {}", e))?;

        let _pkg_receipt = PackageReceipt {
            source_package_sha256: format!(
                "{:x}",
                Sha256::digest(std::fs::read(pkg_path.join("Manifest.json")).unwrap_or_default())
            ),
...
impl BackendLowering for CoreMlLowering {
    ...
    fn validate(&self, region: &ScheduledRegion) -> Result<LegalityReceipt, String> {
        ...
        let _region_id = region.region_id.0;
    ...
    fn lower(&self, region: &ScheduledRegion) -> Result<(Self::Artifact, LoweringReceipt), String> {
        let start = Instant::now();
        ...
        let meta = ModelMeta {
            ...
            outputs: region
                ...
                .iter()
                .enumerate()
                .map(|(i, _tid)| (format!("output_{}", i), vec![1i64, 1]))
                .collect(),
...
mod tests {
    ...
    fn constant_pool_dedup_by_content() {
        ...
        assert!(
            pool.get(&key3).is_none(),
            "different payload must not collide"
        );
    }
    ...
    fn shape_policy_rejects_non_fixed() {
        ...
        .validate()
        .is_err());
        assert!(ShapePolicy::Enumerated {
            default: vec![1, 4],
            alternatives: vec![]
        }
        .validate()
        .is_err());
        assert!(ShapePolicy::Symbolic { named_dims: vec![] }
            .validate()
            .is_err());
    }
