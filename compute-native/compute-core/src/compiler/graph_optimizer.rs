...
impl GraphOptimizer {
    ...
    fn from_plan(plan: &ModelExecutionPlan) -> Self {
        ...
        for (i, layer_plan) in plan.layers.iter().enumerate() {
            ...
            let n_kv = if is_full {
                layer_plan.n_global_kv_heads.unwrap_or(layer_plan.n_kv_heads)
            } else {
            ...
            if layer_plan.q_proj_tensor_id != 0 {
                ...
                opt.known_tensor_shapes.insert(q_out.clone(), q_shape.clone());
                opt.add_op_node(
                    OpKind::QProj,
                    Some(layer),
                    vec![norm_out.clone()],
            ...
            if layer_plan.k_proj_tensor_id != 0 {
                ...
                opt.known_tensor_shapes.insert(k_out.clone(), k_shape.clone());
                opt.add_op_node(
                    OpKind::KProj,
                    Some(layer),
                    vec![norm_out.clone()],
            ...
            if layer_plan.v_proj_tensor_id != 0 {
                ...
                opt.known_tensor_shapes.insert(v_out.clone(), v_shape.clone());
                opt.add_op_node(
                    OpKind::VProj,
                    Some(layer),
                    vec![norm_out.clone()],
            ...
            if layer_plan.o_proj_tensor_id != 0 {
                ...
                opt.known_tensor_shapes.insert(o_out.clone(), o_shape.clone());
                opt.add_op_node(
                    OpKind::OProj,
                    Some(layer),
                    vec![attn_out.clone()],
            ...
            if layer_plan.up_proj_tensor_id != 0 {
                ...
                opt.known_tensor_shapes.insert(up_out.clone(), up_shape.clone());
                opt.add_op_node(
                    OpKind::UpProj,
                    Some(layer),
                    vec![post_norm_out.clone()],
            ...
            if layer_plan.gate_proj_tensor_id != 0 && layer_plan.up_proj_tensor_id != 0 {
                ...
                opt.known_tensor_shapes.insert(mul_out.clone(), mul_shape.clone());
                opt.add_op_node(
                    OpKind::GateTimesUp,
                    Some(layer),
                    vec![
        ...
        if plan.epilogue.final_norm_tensor_id != 0 {
            ...
            opt.known_tensor_shapes.insert(fn_out.clone(), fn_shape.clone());
            opt.add_op_node(
                OpKind::FinalNorm,
                None,
