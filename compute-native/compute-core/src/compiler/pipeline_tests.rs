...
mod tests {
    ...
    use crate::compiler::scheduled::{RegionId, ScheduledModule, ScheduledRegion};
    ...
    use crate::compiler::pass::{NoopPass, TransformPass, TransformPipeline, TransformReceipt};
        ...
        fn validate_region(&self, region: &GraphRegion) -> Result<BackendLegalityReceipt, String> {
            ...
            Ok(BackendLegalityReceipt {
                ...
                violations: if legal {
                    ...
                    vec!["empty region".into()]
                },
                violation_constraint_ids: if legal {
                    vec![]
                } else {
                    vec!["stub:empty".into()]
                },
    ...
    impl TransformPass<SemanticModule> for SemanticToScheduledPass {
        ...
        fn applies_to(&self, _ir: &SemanticModule) -> bool {
            true
        }
    ...
    fn f32_matmul_semantic_to_backend() {
        ...
        let pass: Box<dyn TransformPass<SemanticModule>> = Box::new(SemanticToScheduledPass::new());
        let pipeline = TransformPipeline::new(vec![pass]).with_max_iterations(1);
    ...
    fn pipeline_fixpoint_converges() {
        ...
        impl TransformPass<u32> for ThreeThenStop {
            ...
            fn applies_to(&self, ir: &u32) -> bool {
                *ir < 3
            }
