...
pub fn parse_gguf_header(
    ...
    if &magic != b"GGUF" {
        return Err(format!("Not a GGUF file: magic={magic:?} expected b\"GGUF\""));
    }

    // ── Version ─────────────────────────────────────────────────────────────
    let version = read_u32_le(&mut f)?;
    if version < 1 || version > 3 {
        return Err(format!("Unsupported GGUF version {version} (supported: 1-3)"));
    }
...
fn extract_architecture(metadata: &[(String, String)]) -> Result<crate::config::TextArchitecture, String> {
    ...
    let num_kv_heads = meta_u64(metadata, keys::NUM_KV_HEADS).unwrap_or(num_attention_heads as u64) as u32;
    ...
    let intermediate_size = meta_u64(metadata, keys::INTERMEDIATE_SIZE).unwrap_or(hidden_size as u64 * 4) as u32;
...
mod tests {
    ...
    fn test_parse_gguf_header() {
        ...
        assert!(result.is_ok(), "parse_gguf_header should succeed: {:?}", result.err());
