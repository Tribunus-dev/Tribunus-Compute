use super::{Batch, HardwareConfig, Request, SchedulerConfig, Slot, saved_request::{
    SavedRequest,
    MAX_PREEMPTIONS_BEFORE_BOOST,
...
impl Scheduler {
    ...
    pub fn preempt_lowest(&mut self) -> Option<SavedRequest> {
        ...
        let preemption_count = self.starvation_counters.get(&request_id).copied().unwrap_or(0);
        self.starvation_counters.insert(request_id, preemption_count + 1);
        ...
        let kv_cache_snapshot: Vec<CompressedKvSlot> = kv_cache_pages
            ...
            .map(|(i, _)| {
                ...
                CompressedKvSlot {
                    ...
                    num_tokens: tokens_per_page.min(kv_cache_length.saturating_sub(i * tokens_per_page)),
                }
            })
            .collect();
