...
fn compile_seq(
    ...
    for node in &nodes[1..] {
        ...
        nfa.add_epsilon(nfa.accept, next.start + nfa.states.len().saturating_sub(next.states.len().max(1)));
...
impl GrammarFSM {
    ...
    pub fn advance(&mut self, token_text: &str) -> Result<(), String> {
        for ch in token_text.chars() {
            let next = self.find_transition(self.current_state, ch).ok_or_else(|| {
                format!(
...
fn nfa_to_dfa(nfa: &[NFAState], start_state: usize) -> Vec<DFAState> {
    ...
    while let Some(current_set) = queue.pop_front() {
        ...
        dfa_states[current_id].transitions.dedup_by(|(a, _), (b, _)| a == b);
