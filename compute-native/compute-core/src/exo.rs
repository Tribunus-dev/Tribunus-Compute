...
impl RdmaRegion {
    ...
    pub fn read_at(&self, offset: u64) -> Option<&[u8]> {
        self.allocations.get(&offset).map(|(_, data)| data.as_slice())
}
...
impl DistributedKvCache {
    ...
    fn send_to_remote(&self, _node_idx: usize, _hash: &BlockHash, _data: &[u8]) -> Result<(), String> {
        // Real implementation:
...
impl Autoscaler {
    ...
    pub fn spawn_node(&self) -> Result<(), String> {
        ...
        cmd.spawn().map_err(|e| format!("failed to spawn autoscaled node: {}", e))?;
        Ok(())
    }
...
impl ExoNode {
        ...
        let exo_process = if !no_worker {
            match Self::spawn_exo_worker(port, &hw) {
                ...
                Err(e) => {
                    ...
                    eprintln!("[exo] Continuing without subprocess — EXO must be started manually.");
                    None
                }
            }
        } else {
    ...
    fn spawn_exo_worker(port: u16, hw: &HardwareInfo) -> Result<Child, String> {
        ...
        let child = cmd.spawn().map_err(|e| {
            format!("failed to spawn '{} worker': {}", exo_binary, e)
        })?;
    ...
    fn http_get(url: &str) -> Result<String, String> {
        ...
        let mut stream = TcpStream::connect_timeout(&addr.parse().map_err(|e| format!("parse addr: {}", e))?, timeout)
        ...
        let path = port_path.split('/').skip(1).fold(String::new(), |a, p| {
            if a.is_empty() { format!("/{}", p) } else { format!("{}/{}", a, p) }
        });
    ...
    fn print_banner(chip: &str, addr: &str, ram_gb: u32, rdma: bool) {
        let rdma_status = if rdma { "Thunderbolt 5" } else { "not detected" };
        ...
        eprintln!("    tribunus-server --exo --exo-port {} --model-path /path/to/model\n", port_from_addr(addr));
    }
...
mod tests {
    ...
    fn test_hardware_detection() {
        ...
        eprintln!("Hardware: {} ({} GB, {} ANE cores, RDMA: {})",
    ...
    fn test_consistent_hash_ring_distribution() {
        let ids = vec!["node0".into(), "node1".into(), "node2".into(), "node3".into()];
        let ring = ConsistentHashRing::build(&ids, 151);
    ...
    fn test_sharded_by_node_contains() {
        ...
        sharded.insert(h1, DistributedKvPage {
            token_hash: h1,
            page_data: vec![10],
            source_node: "a".into(),
            rdma_offset: 0,
    ...
    fn test_rdma_region_full() {
        ...
        assert!(result.is_err(), "Allocation exceeding region size should fail");
    ...
    fn test_distributed_kv_cache_put_get_local() {
        let nodes = vec![
            NodeInfo { id: "node0".into(), address: "10.0.0.1:52415".into(), ram_gb: 64, model_layer_range: None, rdma: true },
            NodeInfo { id: "node1".into(), address: "10.0.0.2:52415".into(), ram_gb: 32, model_layer_range: None, rdma: true },
        ];
    ...
    fn test_distributed_kv_cache_check_prefix() {
        let nodes = vec![
            NodeInfo { id: "n0".into(), address: "10.0.0.1:52415".into(), ram_gb: 64, model_layer_range: None, rdma: false },
            NodeInfo { id: "n1".into(), address: "10.0.0.2:52415".into(), ram_gb: 64, model_layer_range: None, rdma: false },
        ];
