...
pub(crate) fn load_tensor_from_mapped_segment(
    ...
    match entry.storage_dtype.as_str() {
        "U8" | "Uint8" => unsafe {
            let arr = crate::external_array::new_external_array(
                storage,
                &dims,
                mlx_rs::Dtype::Uint8,
            )
...
impl LoadedProfiledModel {
    ...
    pub fn new(image_dir: &Path) -> crate::Result<Self> {
        ...
        let segment_views: Vec<crate::mapped_image::SegmentView> = reader
            ...
            .collect();
        let mapped_image =
            crate::mapped_image::MappedImage::open_mapped(image_dir, &segment_views)
        ...
        let memory_island = SharedMemoryIsland::new();
        ...
        let ane_cache = {
            ...
            if orion_indices.is_empty() {
                ...
                let mut segments_vec: Vec<Arc<crate::mapped_image::MappedSegment>> = Vec::new();
        ...
        for (l, layer_plan) in reader.manifest.execution_plan.layers.iter().enumerate() {
            ...
            if layer_plan.attention_kind != "full_attention" {
                match crate::compiler::ane::kv_decompress_program::compile_mil_text(
                    layer_plan.n_heads,
            ...
            .any(|e| e.name.contains("vision_encoder"))
        {
            ...
            let mut load_vision_tensor =
                |name: &str| -> Result<Arc<Array>, String> {
                    if let Some(entry) =
                        reader.manifest.tensor_table.iter().find(|e| e.name == name)
                    {
                        let seg_id = &entry.segment;
                        let segment = mapped_image.segments.get(seg_id).ok_or_else(|| {
                        ...
                        let (arr, _classification) =
                            load_tensor_from_mapped_segment(segment, entry, false)
