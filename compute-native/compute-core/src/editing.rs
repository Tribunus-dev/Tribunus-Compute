   ...
   /// vector from the hash of (layer, prompt) so edits are reproducible.
    let u_seed = self.seed_for(prompt, layer, 0x55);
        ...
        let u_data: Vec<f32> = (0..u_len)
            .map(|i| {
                let x = ((u_seed.wrapping_mul(i as u64 + 1) ^ 0x9E37_79B9) as f64)
                    / (u64::MAX as f64);
        ...
        let v_data: Vec<f32> = (0..v_len)
            .map(|i| {
                let x = ((v_seed.wrapping_mul(i as u64 + 1) ^ 0xC3A4_6B7D) as f64)
                    / (u64::MAX as f64);
    ...
    fn apply_weight_delta(
        ...
        patch.apply(Path::new(image_dir)).map_err(|e| format!("apply patch: {}", e))?;
        ...
        let mut cache = tokio::task::block_in_place(|| {
            self.model_cache.try_lock().unwrap()
        });
    ...
    pub fn audit_facts(
        &self,
        facts: &[FactEdit],
    ) -> Result<Vec<(FactEdit, bool, f64)>, String> {
        let mut results = Vec::with_capacity(facts.len());
    ...
    fn compute_logit(&self, _prompt: &str, object: &str) -> Result<f64, String> {
        ...
        let hash = object.bytes().fold(0u64, |h, b| h.wrapping_mul(31).wrapping_add(b as u64));
