...
fn create_2x3_f32(backend: &mut dyn TensorBackend) -> TensorHandle {
    ...
    backend.create_f32(&data, &[2, 3]).expect("create 2×3 f32")
}
...
fn create_3x2_f32(backend: &mut dyn TensorBackend) -> TensorHandle {
    ...
    backend.create_f32(&data, &[3, 2]).expect("create 3×2 f32")
}
...
fn shape_does_not_trigger_eval() {
    ...
    assert_eq!(
        shape,
        vec![2, 2],
        "shape of matmul(2×3, 3×2) should be [2,2]"
    );
...
fn matmul_numerical_parity() {
    ...
    assert!(
        readback.forced_eval,
        "first readback after eval may not force new eval"
    );
    ...
    assert!(
        (result[0] - 58.0).abs() < 0.01,
        "c[0,0] should be 58, got {}",
        result[0]
    );
    assert!(
        (result[1] - 64.0).abs() < 0.01,
        "c[0,1] should be 64, got {}",
        result[1]
    );
    assert!(
        (result[2] - 139.0).abs() < 0.01,
        "c[1,0] should be 139, got {}",
        result[2]
    );
    assert!(
        (result[3] - 154.0).abs() < 0.01,
        "c[1,1] should be 154, got {}",
        result[3]
    );
...
fn grouped_evaluation_one_fence() {
    ...
    assert_eq!(
        receipt.eval_calls, 1,
        "single dependency chain should need 1 eval"
    );

    // observed_substrate must be None until Phase 2 native instrumentation
    assert!(
        receipt.observed_substrate.is_none(),
        "substrate must be None until Phase 2"
    );
...
fn grouped_evaluation_two_independent_outputs() {
    ...
    assert_eq!(
        receipt.eval_calls, 2,
        "two independent outputs should need 2 eval calls"
    );
    assert!(receipt.sync_ns > 0);
...
fn backend_reports_capabilities() {
    ...
    assert!(
        !caps.supports_bf16_native,
        "MLX backend does not support native BF16"
    );
}
...
fn matmul_rejects_inner_dimension_mismatch() {
    ...
    let msg = err.unwrap_err();
    assert!(
        msg.contains("K mismatch") || msg.contains("inner dimensions"),
        "got: {}",
        msg
    );
