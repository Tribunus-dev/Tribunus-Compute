...
async fn commands_execute_on_lane_thread() {
    ...
    let handle = spawn_lane(ComputeLaneId(1), test_device(), 4, move |mut rx| {
...
async fn queue_capacity_produces_backpressure() {
    ...
    let handle = spawn_lane(ComputeLaneId(2), test_device(), 1, move |mut rx| {
        for _ in 0..2 {
            match rx.blocking_recv() {
                Some(ComputeCommand::Status {
                    request_id: _,
    ...
    let (tx1, rx1) = oneshot::channel::<LaneStatus>();
    handle
        .send(ComputeCommand::Status {
        ...
        .await
        .expect("send first");
    let (tx2, rx2) = oneshot::channel::<LaneStatus>();
    handle
        .send(ComputeCommand::Status {
...
async fn cancel_prevents_subsequent_decode() {
    ...
    let handle = spawn_lane(ComputeLaneId(3), test_device(), 4, move |mut rx| {
        loop {
            match rx.blocking_recv() {
                Some(ComputeCommand::Cancel {
                    request_id: _,
                    session_id: _,
                    reason: _,
                }) => {
    ...
    assert!(
        event.is_none(),
        "cancelled decode should not produce tokens"
    );
...
async fn shutdown_closes_admission() {
    let handle = spawn_lane(ComputeLaneId(4), test_device(), 4, move |mut rx| {
        match rx.blocking_recv() {
