...
impl<T> ToNapiResult<T>
    for std::result::Result<T, tribunus_compute_core::engine_error::EngineError>
{
    fn to_napi(self) -> napi::Result<T> {
        self.map_err(|e| napi::Error::from_reason(format!("{:?}", e)))
    }
...
pub fn create_array_raw(
    ...
    bridge::create_array_raw(&data, &shape, dtype_id)
        .map(|h| h as i64)
        .to_napi()
}
...
pub fn array_size(handle: i64) -> napi::Result<u32> {
    bridge::array_size(handle as u64)
        .map(|s| s as u32)
        .to_napi()
}
#[napi]
pub fn array_nbytes(handle: i64) -> napi::Result<u32> {
    bridge::array_nbytes(handle as u64)
        .map(|s| s as u32)
        .to_napi()
}
...
pub fn array_data_f32(
    ...
    bridge::array_data_f32(handle as u64, out.as_mut())
        .map(|n| n as u32)
        .to_napi()
}
...
pub fn matmul(a: i64, b: i64) -> napi::Result<i64> {
    bridge::matmul(a as u64, b as u64)
        .map(|h| h as i64)
        .to_napi()
}
#[napi]
...
pub fn multiply(a: i64, b: i64) -> napi::Result<i64> {
    bridge::multiply(a as u64, b as u64)
        .map(|h| h as i64)
        .to_napi()
}
...
pub fn run_full_model_from_image(
    ...
    let reader =
        compute_image::CompiledImageReader::open(std::path::Path::new(&image_dir)).to_napi()?;
