...
//!
//! Calls mlx_array_new_external in the forked mlx-c layer, which wraps
...
pub unsafe fn new_external_array(
    ...
    // The C callback trampoline — must be extern "C" with no unwind.
    extern "C" fn deleter_trampoline(ptr: *mut c_void, ctx: *mut c_void) {
        if ctx.is_null() {
            return;
        }
        // Re-box the context and call the release function.
        let ctx: Box<DeleterContext> = unsafe { Box::from_raw(ctx as *mut DeleterContext) };
        // Catch panics to prevent unwind through C.
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| unsafe {
            (ctx.release)(ctx.ptr, ctx.byte_len);
        }));
        // ctx dropped here → box freed.
    }

    let ctx_ptr = Box::into_raw(ctx) as *mut c_void;

    // Safety: ctx_ptr is a valid Box<DeleterContext> converted to raw.
    // The trampoline is a valid C function pointer.
    // shape and dtype are well-formed.
    let mut arr_handle: mlx_sys::mlx_array = unsafe { std::mem::zeroed() };
    let status = unsafe {
        mlx_sys::mlx_array_new_external(
            &mut arr_handle,
            data,
            shape.as_ptr(),
            shape.len() as i32,
    ...
    if status != 0 {
        ...
        let _ = unsafe { Box::from_raw(ctx_ptr as *mut DeleterContext) };
        return Err(mlx_rs::error::Exception::custom(
            "mlx_array_new_external failed",
        )
        .into());
    }

    // Safety: mlx_array_new_external succeeded, arr_handle is valid.
    Ok(unsafe { Array::from_ptr(arr_handle) })
}

fn dtype_to_mlx_dtype(dtype: Dtype) -> i32 {
    match dtype {
        Dtype::Float32 => 10,
        Dtype::Float16 => 9,
        Dtype::Bfloat16 => 12,
