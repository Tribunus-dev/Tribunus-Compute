...
/// Wrap a pre-existing IOSurface for use with the ANE runtime.
/// The surface is retained (CFRetain) — the caller can CFRelease its reference
/// immediately after calling this function. Returns the same surface on success,
/// NULL on null input.
/// See: IosurfaceAllocator in compute-core (shared memory island)
IOSurfaceRef orion_tensor_from_external(IOSurfaceRef surface);

/// Create an IOSurface using the Arena pixel format (kCVPixelFormatType_OneComponent16Half)
/// compatible with both MLX external arrays (via new_external_array) and Core ML
/// multi-arrays (via CVPixelBuffer wrapping). Uses the Arena dimension convention:
/// width=seq_len, height=channels, bytesPerElement=2. This matches the shared memory
/// island's IOSurfaceFp16ContiguousV1 profile.
/// @param channels Number of channels (C dimension, also height)
/// @param seq_len  Sequence length (S dimension, also width)
/// @return IOSurface reference, or NULL on failure. Caller must CFRelease.
/// See: Arena::new() in compute-core, IosurfaceAllocator
IOSurfaceRef orion_tensor_from_arena(int channels, int seq_len);
