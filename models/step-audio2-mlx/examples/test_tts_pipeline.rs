...
fn main() -> Result<()> {
    ...
    if !s3_weights.exists() {
        println!("  S3Tokenizer weights not found. Run: python scripts/convert_s3tokenizer.py");
    ...
    if !flow_weights.exists() {
        println!("  Flow weights not found. Run: python scripts/convert_tts_weights.py");
    ...
    if !hifigan_weights.exists() {
        println!("  HiFi-GAN weights not found. Run: python scripts/convert_tts_weights.py");
